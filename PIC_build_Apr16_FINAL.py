...
function exportSchedulePDF(id){
  ...
  win.document.write('<!DOCTYPE html><html><head><title>Service Schedule - '+p.name+'</title>'
    ...
    +'<br><button onclick="window.print()" style="padding:10px 20px;background:#1e3a5f;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:12px">Print / Save PDF</button>'
    +'</body></html>');
  win.document.close();
...
function setSvcType(t){
  ...
  document.querySelectorAll('.svctype-btn').forEach(b=>{
    const isOn=b.id==='svctype-'+t.replace('_','-').replace('60','60');
