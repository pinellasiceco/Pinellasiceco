...
*Last updated: 2026-05-17 (session 36 — data pipeline fixes + rebuild.yml root cause fix) by Claude Code*
...
## Recent Changes
- **2026-05-17 (s36 — data pipeline fixes + rebuild.yml root cause fix):** `download_data.py`: after downloading `3fdinspi_current.csv`, reads column 14 (inspection date), finds the max date, and prints data lag in days to CI logs — every rebuild now shows data freshness automatically. `send_briefing.py`: added `engine='openpyxl'` to `pd.read_excel()` for EOS Weekly Extract — fixes "Excel file format cannot be determined" error. **Root cause fix:** `rebuild.yml` was pulling `supabase/functions/` from `claude/fix-multiple-app-bugs-j1hpJ` on every daily rebuild, overwriting the real Stripe price IDs with `price_xxx` placeholders every run. Removed that line — `supabase/functions/` is now on main and no longer needs to be pulled from the feature branch.
- **2026-05-17 (s35 — Stripe checkout complete, pending Stripe account activation):** Completed session 34 work. Replaced `price_xxx` placeholders in `supabase/functions/stripe-checkout/index.ts` with real Stripe price IDs (entry_fee `price_1TXrHM1DW5dOU2aay60IOPnW`, monthly_base `price_1TXrIG1DW5dOU2aaqhWTDpHG`, quarterly_base `price_1TXrIu1DW5dOU2aacIeDByPC`, monthly_additional `price_1TXrZg1DW5dOU2aalDMRdtD1`, quarterly_additional `price_1TXra61DW5dOU2aaOUH8FbBa`, onetime_base `price_1TXrJQ1DW5dOU2aa4jQguT8b`, onetime_additional `price_1TXral1DW5dOU2aaXB5myZ8O`). Note: checkout uses dynamic `price_data` (computed per-request) — these IDs are stored as reference. Added `claude/stripe-checkout-edge-function-BiUpp` to `deploy_edge_functions.yml` branch trigger. Added `--use-api` flag to both deploy steps to avoid Docker dependency in CI. Added `SUPABASE_PROJECT_REF` and refreshed `SUPABASE_ACCESS_TOKEN` in GitHub repo secrets. Edge Function deployed to Supabase. **Pending:** Stripe account confirmation email — once activated, Close Deal → Charge Now / Send to Client will be fully live with no further code changes needed.
