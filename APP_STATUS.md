...
*Last updated: 2026-05-18 (session 43 — fix Stripe return monthly price bug) by Claude Code*
...
## Recent Changes
- **2026-05-18 (s43 — fix Stripe return monthly price bug):** Two bugs in the post-Stripe-payment close flow: (1) `checkStripeReturn()` rebuilt `_closeState` using `p.machines` (DBPR raw estimate) instead of the machine count the user configured in the Close Deal overlay — if DBPR showed 2 machines but user sold a 1-machine plan, the MRR stored was 2-machine rate. (2) `scMarkWon()` called `calcMonthly()` with no discount applied — plan discounts set in the overlay were ignored in the stored MRR. Fix: `generateStripeCheckout()` now saves `_pending_machines` and `_pending_plan_disc` to `customers[pid]` before the Stripe redirect; `checkStripeReturn()` restores those values into `_closeState`; `scMarkWon()` reads `cs.planDisc` (Stripe path) or the `co-plan-disc` input (direct Mark Won path) and subtracts it from the monthly price.
...
- **2026-05-18 (s40 — Home tab lead triage overhaul):** Fresh Citations section (top of Home tab): surfaces DBPR-cited prospects with `cit_latest` within 72 hours, ignores recency filter, shows citation date label, inspector observation snippet, prev-contact badge. Unclaimed Citations: DBPR-cited, zero log entries, citations 3–30 days old. In Play section restructured: only Overdue + Today shown, per-card "X days overdue" label with escalation coloring (red 7+d, amber 3+d). `buildTodaysPlan()` excludes Fresh/Unclaimed via `isHandledElsewhere()`. `viewAllFilter(key)` switches to Prospects tab with dismissible blue filter banner; `applyTempFilter()` / `clearTempFilter()` manage state; `_activeFilter` in `rA()` overrides preset for all 5 filter keys. Removed: Cold Targets, Strike Zone, New Since Yesterday sections + all dead code. Stripe: fixed `label.custom` 50-char limit (shortened to ≤38 chars); fixed `customer_creation: always` not valid in subscription mode. iOS PWA: `coSendToClient` now shows payment link inline when no phone number on record. Stripe test mode: swap `STRIPE_SECRET_KEY` in Supabase secrets; no redeploy needed.
- **2026-05-17 (s39 — re-test reminder system + referral line):** 60-day re-test auto-scheduling: `logServiceFromCal()` detects first visit for non-subscribers and sets `customers[pid].retest_scheduled` (+60 days) and `retest_done: false`, then fires a secondary toast at +1200ms. `renderRetestSection()` + `buildRetestCard()` add a "🔬 Re-Tests Due" section to the Home tab (after nudge section) showing any prospect with a re-test due within ±7 days — cards show first-visit ATP reading, days-until label (red/amber/gray), and Log Re-Test + Dismiss buttons. `openRetestLog()` opens a bottom-sheet with the first-visit comparison, ATP input, and notes. `saveRetestLog()` saves `retest_done/atp/date`, then shows a result sheet with before/after side-by-side and a gold "Convert to Plan" button that calls `scOpenClose()`. `dismissRetest()` marks done and re-renders the wrapper in place. All buttons iOS-safe via `data-*` attributes. Referral line added to `srSendEmail()` only (not `srGenerate`/`scStatusReport` prospect pitch): "Know a restaurant owner who could use this? Forward this report — we'll take care of them." with tappable `tel:` link.
- **2026-05-17 (s38 — gallery images live):** Photos were uploaded to repo root with original iPhone names (IMG_0726–IMG_0736). Moved all 10 to `docs/before-after/img/` with descriptive names matching their card slots (distributor, evaporator-plates, machine-frame, compressor, condenser, water-valve, ice-grid, pipes, evaporator, pump). Updated all 10 `<img src>` paths in `docs/before-after/index.html` to match the actual file extensions (webp/jpeg/png) — GitHub Pages is case-sensitive and `.jpg` ≠ `.jpeg`. Root-level originals removed.
- **2026-05-17 (s37 — Flex Plan toggle):** Added Flex Plan checkbox to the Close Deal overlay (between Year 1 Total and Discounts sections). Unchecked by default every time the overlay opens. When checked, `generateStripeCheckout()` sends `flex: true` to the Edge Function. Edge Function swaps the Stripe T&C custom_fields label to month-to-month language ("Cancel anytime with 30 days written notice") and adds `flex: 'true'/'false'` to session metadata and subscription_data metadata so it shows in Stripe Dashboard. Standard 12-month terms remain the default. Back-pocket tool — show to prospects who resist commitment.
- **2026-05-17 (s36 — data pipeline fixes + rebuild.yml permanent fix):** `download_data.py`: prints most recent inspection date + data lag after CSV download. `send_briefing.py`: `engine='openpyxl'` fix for EOS Excel read. `rebuild.yml`: removed all stale `git checkout origin/claude/fix-multiple-app-bugs-j1hpJ` lines except `atp/` — that branch is now older than main and was reverting build.py, send_briefing.py, stripe-checkout/index.ts, deploy_edge_functions.yml on every CI run.
- **2026-05-17 (s35 — Stripe checkout complete, pending Stripe account activation):** Replaced `price_xxx` placeholders with real Stripe price IDs. Added `--use-api` flag to deploy steps. Added `SUPABASE_PROJECT_REF` / `SUPABASE_ACCESS_TOKEN` secrets. Edge Function deployed. Pending Stripe account confirmation.
- **2026-05-16 (s34 — Stripe checkout integration):** Dynamic Stripe payment flow wired into the Close Deal overlay. New Edge Function `supabase/functions/stripe-checkout/index.ts`. Close Deal overlay: discount fields, Send to Client / Charge Now buttons, Mark Won fallback. New JS functions: `generateStripeCheckout()`, `coSendToClient()`, `coChargeNow()`, `checkStripeReturn()`.
