...
*Last updated: 2026-05-17 (session 36 — data pipeline fixes) by Claude Code*
...
## Recent Changes
- **2026-05-17 (s36 — data pipeline fixes):** `download_data.py`: after downloading `3fdinspi_current.csv`, reads column 14 (inspection date), finds the max date, and prints the data lag in days to CI logs — every rebuild now shows freshness automatically. `send_briefing.py`: added `engine='openpyxl'` to `pd.read_excel()` call for EOS Weekly Extract — fixes "Excel file format cannot be determined" error; `openpyxl` already present in CI pip install.
