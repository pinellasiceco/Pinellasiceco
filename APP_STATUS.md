...
- **Stripe redirect wipes in-memory state**: Any data set on `customers[pid]` in memory before `window.location.href = stripeUrl` is lost — Stripe navigates away and the app reloads from localStorage. Always call `custSave()` immediately after writing anything to `customers[pid]` that needs to survive the redirect. Same applies to any other page-navigation flow.
