...
function showCard(id){
  ...
  toast('Loading '+p.name.slice(0,18)+'...');
  ...
  const phoneH=ph
    ?'<a href="tel:'+ph.replace(/\D/g,'')+'" style="display:inline-flex;align-items:center;gap:8px;background:#059669;color:#fff;padding:10px 16px;border-radius:8px;font-size:14px;font-weight:700;text-decoration:none;touch-action:manipulation;margin-bottom:10px">PHONE '+ph+'</a>'
    :'<div style="margin-bottom:10px"><a href="https://www.google.com/search?q='+enc(p.name+' '+p.city+' FL phone')+'" target="_blank" style="font-size:12px;color:var(--blu);border:1px solid var(--blu);padding:6px 12px;border-radius:6px;text-decoration:none">Find Phone</a></div>';

  const pBg={CALLBACK:'#fef2f2',HOT:'#fff7ed',WARM:'#fefce8',COOL:'#f8fafc'};
  const pCl={CALLBACK:'#dc2626',HOT:'#f97316',WARM:'#ca8a04',COOL:'#64748b'};
  const pc=pCl[p.priority]||'#64748b';
  const lColor=OI_COLOR[normO(lc&&lc.outcome)]||'#64748b';
  const chips=[
  ...
  const intel=[
    p.n_callbacks>0?'CALLBACK x'+p.n_callbacks+' callback inspection'+(p.n_callbacks>1?'s':''):'',
    p.ice_count>0?'ICE x'+p.ice_count+' ice violation'+(p.ice_count>1?'s':'')+' on record':'',
    p.high_viol>0?'WARN '+p.high_viol+' critical violation'+(p.high_viol>1?'s':''):'',
    p.codes&&p.codes.length?'Codes: '+p.codes.slice(0,5).join(', '):'',
    p.pred_next?'Next predicted: '+p.pred_next:'',
    p.confidence?'Confidence: '+p.confidence+'%':'',
    '$'+(p.monthly||149)+'/mo',
  ].filter(Boolean);

  const intelH=intel.length
    ?'<div style="background:#f8fafc;border-radius:8px;padding:10px;margin-bottom:12px;border-left:3px solid #e2e8f0">'
      +intel.map(function(r){return '<div style="font-size:10px;color:#475569;line-height:2">'+r+'</div>';}).join('')
     +'</div>':'';

  const pitchFn=PITCHES[p.pitch_type]||PITCHES.routine;
  const walkinFn=WALKIN[p.pitch_type]||WALKIN.routine;

  const pitchH='<div style="margin-bottom:12px"><div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;margin-bottom:5px">PITCH OPENER</div>'
    +'<div style="background:#fff7ed;border-left:3px solid #c9973a;padding:10px;font-size:11px;color:#475569;line-height:1.7">'+pitchFn(p.name)+'</div></div>';

  const walkinH='<div style="margin-bottom:12px"><div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;margin-bottom:5px">WALK-IN SCRIPT</div>'
    +'<div style="background:#f0fdf4;border-left:3px solid #059669;padding:10px;font-size:11px;color:#475569;line-height:1.7">'+walkinFn(p.name)+'</div></div>';

  const histH=entries.length
    ?'<div style="margin-bottom:12px"><div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;margin-bottom:6px">HISTORY</div>'
      +[...entries].reverse().slice(0,5).map(function(e){
        var ec=OI_COLOR[normO(e.outcome)]||'#64748b';
        return '<div style="font-size:11px;padding:4px 0;border-bottom:1px solid #f1f5f9">'
          +'<span style="color:'+ec+';font-weight:700">'+(OI[normO(e.outcome)]||e.outcome)+'</span>'
          +' <span style="color:#94a3b8">'+e.date+'</span>'
          +(e.notes?' <em style="color:#64748b">'+e.notes.slice(0,50)+'</em>':'')
         +'</div>';
      }).join('')+'</div>':'';

  const objH='<div style="margin-bottom:12px"><div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;margin-bottom:6px">OBJECTION HANDLERS</div>'
    +OBJECTIONS.slice(0,4).map(function(o){
      return '<details style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;margin-bottom:4px">'
        +'<summary style="padding:7px 10px;font-size:11px;font-weight:600;color:#0f1f38;cursor:pointer;list-style:none">? '+o.q+'</summary>'
        +'<div style="padding:6px 10px 10px;font-size:10px;color:#475569;line-height:1.6;border-top:1px solid #e2e8f0">'+o.a+'</div>'
        +'</details>';
    }).join('')+'</div>';

  // Build outcome buttons using data attributes - NO inline onclick with quotes
  var outcomes=[
    {label:'Intro Set',  outcome:'intro_set', bg:'#ecfdf5',color:'#059669',brd:'#6ee7b7'},
    {label:'In Play',    outcome:'in_play',   bg:'#fffbeb',color:'#d97706',brd:'#fcd34d'},
    {label:'No Contact', outcome:'no_contact',bg:'#eff6ff',color:'#3b82f6',brd:'#93c5fd'},
    {label:'Not Now',    outcome:'not_now',   bg:'#fef2f2',color:'#dc2626',brd:'#fca5a5'},
    {label:'Voicemail',  outcome:'voicemail', bg:'#f5f3ff',color:'#7c3aed',brd:'#c4b5fd'},
    {label:'Dead',       outcome:'dead',      bg:'#f1f5f9',color:'#475569',brd:'#94a3b8'},
  ];

  var outcomeBtns='<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:8px" id="sc-outcome-row">'
    +outcomes.map(function(o){
      return '<button data-scid="'+id+'" data-scout="'+o.outcome+'" class="sc-btn"'
        +' style="padding:10px 4px;border:1px solid '+o.brd+';border-radius:8px;background:'+o.bg+';color:'+o.color+';font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation">'+o.label+'</button>';
    }).join('')+'</div>';

  var overlay=document.createElement('div');
  overlay.id='sc-overlay';
  overlay.style.cssText='position:fixed;inset:0;z-index:300;background:rgba(15,31,56,.7);display:flex;align-items:flex-end;justify-content:center';

  overlay.innerHTML=
    '<div id="sc-sheet" style="background:#fff;width:100%;max-width:600px;max-height:92vh;border-radius:16px 16px 0 0;overflow-y:auto;-webkit-overflow-scrolling:touch">'
    +'<div style="position:sticky;top:0;background:#fff;border-bottom:1px solid #e2e8f0;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;z-index:1">'
      +'<div>'
        +'<div style="font-size:16px;font-weight:800;color:#0f1f38">'+p.name+'</div>'
        +'<div style="font-size:10px;color:#94a3b8">'+p.address+', '+p.city+'</div>'
      +'</div>'
      +'<button id="sc-close" style="border:none;background:#f1f5f9;border-radius:50%;width:36px;height:36px;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;touch-action:manipulation;flex-shrink:0">x</button>'
    +'</div>'
    +'<div style="padding:16px">'
      +'<div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:12px">'+chips+'</div>'
      +phoneH+intelH+pitchH+walkinH+histH+objH
      +'<div style="font-size:11px;font-weight:700;color:#0f1f38;margin-bottom:8px">Log Outcome</div>'
      +outcomeBtns
      +'<textarea id="sc-notes" placeholder="Notes (optional)..." rows="2" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:7px;font-size:12px;font-family:inherit;resize:none;outline:none;box-sizing:border-box"></textarea>'
    +'</div></div>';

  document.body.appendChild(overlay);

  // Wire close button
  document.getElementById('sc-close').addEventListener('touchend',function(e){
    e.preventDefault();
    overlay.remove();
  });
  ...
  // Wire outcome buttons via delegation on the grid
  document.getElementById('sc-outcome-row').addEventListener('touchend',function(e){
    e.preventDefault();
    var btn=e.target.closest('.sc-btn');
    if(!btn)return;
    scLog(btn.dataset.scid, btn.dataset.scout);
  });
  document.getElementById('sc-outcome-row').addEventListener('click',function(e){
    var btn=e.target.closest('.sc-btn');
    if(!btn)return;
    scLog(btn.dataset.scid, btn.dataset.scout);
  });
...
function queueLogFull(outcome){
  ...
  _queueAutoAdvance=true; // flag: after lSave, advance queue
  openM(p.id);
...
function openM(id){
  const p=P.find(x=>x.id===id);if(!p){toast('Prospect not found');return;}
  try{
...
// INIT
function init(){
  lLoad();phLoad();custLoad();contactsLoad();initSettings();initGoals();renderBriefing();
