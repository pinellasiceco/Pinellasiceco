...
function cardHTML(p){
  ...
  return '<div class="card '+p.priority+(isC(p.id)?' done':'')+'" data-id="'+p.id+'">'
    ...
    +'<div class="cacts">'
      ...
      +'<button class="btn" data-id="'+p.id+'" data-action="openM" onclick="event.stopPropagation();openM('+p.id+')" ontouchend="event.stopPropagation();event.preventDefault();openM('+p.id+')" style="background:#0f1f38;color:#fff;border-color:#0f1f38;touch-action:manipulation">Details</button>'
