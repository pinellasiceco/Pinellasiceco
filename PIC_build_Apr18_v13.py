...
function cardHTML(p){
  ...
  return '<div class="card '+p.priority+(isC(p.id)?' done':'')+'" data-id="'+p.id+'">'
    ...
    +lastH
    +'<div class="cacts">'
      +'<button class="btn blog" class="log-call-btn" data-id="'+p.id+'" data-action="openM">Log Call</button>'
...
function attachGridListeners(grid){
  ...
  // Single touchend listener on the container — works on ALL iOS versions
  ...
  fresh.addEventListener('touchend',function(e){
    ...
    // Find the action button first (Log Call, +Route, Skip, Find, Save)
  ...
  // Click fallback for non-touch (desktop/simulator)
...
function renderQueueCard(){
  ...
  const phoneHTML=phone
    ?'<a href="tel:'+phone.replace(/\D/g,'')+'" '
      +'style="display:inline-flex;align-items:center;gap:8px;background:#059669;color:#fff;'
      +'padding:11px 18px;border-radius:10px;font-size:15px;font-weight:700;text-decoration:none;'
      +'touch-action:manipulation;margin-bottom:10px">📞 '+phone+'</a>'
    :'<div style="display:flex;gap:8px;align-items:center;margin-bottom:10px">'
      +'<span style="font-size:12px;color:#94a3b8">No phone on file</span>'
      +'<a href="https://www.google.com/search?q='+enc(p.name+' '+p.city+' FL phone')+'" target="_blank" '
       +'style="font-size:11px;color:var(--blu);text-decoration:none;padding:4px 8px;border:1px solid var(--blu);border-radius:5px">Find</a>'
     +'</div>';

  // ── Chips ─────────────────────────────────────────────────────────────────
  const pColors={'CALLBACK':'#dc2626','HOT':'#f97316','WARM':'#ca8a04','COOL':'#64748b'};
  const pBg    ={'CALLBACK':'#fef2f2','HOT':'#fff7ed','WARM':'#fefce8','COOL':'#f8fafc'};
  const pColor=pColors[p.priority]||'#64748b';
  const pBgCol=pBg[p.priority]||'#f8fafc';

  const lastStatus=lc?(OI[normO(lc.outcome)]||lc.outcome):'Not contacted';
  const lastColor=OI_COLOR[normO(lc?.outcome)]||'#64748b';

  const chips=[
    '<span style="font-size:9px;padding:3px 8px;border-radius:5px;font-weight:700;'
      +'background:'+pBgCol+';color:'+pColor+';border:1px solid '+pColor+'40">'+p.priority+'</span>',
    p.chronic
      ?'<span style="font-size:9px;padding:3px 8px;border-radius:5px;font-weight:700;background:#ecfdf5;color:#059669;border:1px solid #6ee7b740">CHRONIC ICE</span>'
      :'',
    p.days_since>0
      ?'<span style="font-size:9px;padding:3px 8px;border-radius:5px;font-weight:700;color:#dc2626;background:#fef2f2;border:1px solid #fca5a540">'+p.days_since+'d overdue</span>'
      :p.days_until>=0
        ?'<span style="font-size:9px;padding:3px 8px;border-radius:5px;font-weight:700;color:#059669;background:#ecfdf5;border:1px solid #6ee7b740">'+p.days_until+'d until inspection</span>'
        :'',
    lc
      ?'<span style="font-size:9px;padding:3px 8px;border-radius:5px;font-weight:700;color:'+lastColor+';background:'+lastColor+'15;border:1px solid '+lastColor+'40">'+lastStatus+'</span>'
      :'',
  ].filter(Boolean).join('');
  ...
  const intelRows=[
    ...
    (p.codes&&p.codes.length) ? 'Codes: '+p.codes.slice(0,4).join(', ') : '',
    p.pred_next      ? 'Next inspection predicted: '+p.pred_next : '',
  ].filter(Boolean);

  const intelHTML=intelRows.length
    ?'<div style="background:#f8fafc;border-radius:8px;padding:8px 10px;margin-bottom:10px">'
      +intelRows.map(r=>'<div style="font-size:10px;color:#475569;line-height:1.8">'+r+'</div>').join('')
     +'</div>'
    :'';

  // ── Last contact note ─────────────────────────────────────────────────────
  const lastNote=lc&&lc.notes
    ?'<div style="font-size:10px;color:#64748b;font-style:italic;padding:6px 8px;background:#f1f5f9;border-radius:6px;border-left:3px solid var(--brd);margin-bottom:10px">'
      +'\u201c'+lc.notes+'\u201d &mdash; '+lc.date
     +'</div>'
    :'';
  ...
  const histHTML=entries.length
    ?'<div style="margin-bottom:10px">'
      +'<div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Recent history</div>'
      +[...entries].reverse().slice(0,3).map(e=>{
        const ec=OI_COLOR[normO(e.outcome)]||'#64748b';
        return '<div style="display:flex;gap:6px;align-items:baseline;font-size:10px;line-height:1.8">'
          +'<span style="color:'+ec+';font-weight:700">'+(OI[normO(e.outcome)]||e.outcome)+'</span>'
          +'<span style="color:#94a3b8">&bull; '+e.date+'</span>'
          +(e.notes?'<span style="color:#64748b;font-style:italic"> &mdash; '+e.notes.slice(0,35)+'</span>':'')
         +'</div>';
      }).join('')
     +'</div>'
    :'';
  ...
  wrap.innerHTML=
    ...
    +'<div style="margin-bottom:8px">'
      +'<div style="font-size:16px;font-weight:800;color:var(--navy);margin-bottom:2px;line-height:1.2">'+p.name+'</div>'
    ...
    +'<textarea id="q-notes" placeholder="Quick note (optional)..." rows="2" '
      +'style="width:100%;padding:8px;border:1px solid var(--brd);border-radius:7px;'
