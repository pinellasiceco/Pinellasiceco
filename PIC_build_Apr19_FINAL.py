...
function showCard(id){
  ...
  // Phone
  var phoneH=ph
    ?'<a href="tel:'+ph.replace(/\D/g,'')+'" style="display:flex;align-items:center;gap:8px;background:#059669;color:#fff;padding:10px 16px;border-radius:8px;font-size:14px;font-weight:700;text-decoration:none;touch-action:manipulation;margin-bottom:10px">📞 '+ph+'</a>'
    :'<div style="margin-bottom:10px"><span style="font-size:11px;color:#94a3b8">No phone on file</span> <a href="https://www.google.com/search?q='+enc(p.name+' '+p.city+' FL phone')+'" target="_blank" style="margin-left:8px;font-size:11px;color:var(--blu);border:1px solid var(--blu);padding:3px 8px;border-radius:5px;text-decoration:none">Find \u2197</a></div>';
  ...
  var pitchH='<div style="margin-bottom:12px">'
    +'<div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px">Pitch Opener</div>'
    +'<div style="background:#fff7ed;border-left:3px solid #c9973a;border-radius:0 8px 8px 0;padding:10px;font-size:11px;color:#475569;line-height:1.7">'+pitchFn(p.name)+'</div>'
  +'</div>';
  var walkinH='<div style="margin-bottom:12px">'
    +'<div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px">Walk-In Script</div>'
    +'<div style="background:#f0fdf4;border-left:3px solid #059669;border-radius:0 8px 8px 0;padding:10px;font-size:11px;color:#475569;line-height:1.7">'+walkinFn(p.name)+'</div>'
  +'</div>';

  // Objections
  var objH='<div style="margin-bottom:12px">'
    +'<div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px">Objection Handlers</div>'
    +OBJECTIONS.map(function(o){
      return '<details style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;margin-bottom:4px">'
        +'<summary style="padding:7px 10px;font-size:10px;font-weight:600;color:#0f1f38;cursor:pointer;list-style:none">\u2753 '+o.q+'</summary>'
        +'<div style="padding:6px 10px 10px;font-size:10px;color:#475569;line-height:1.6;border-top:1px solid #e2e8f0">'+o.a+'</div>'
        +'</details>';
    }).join('')
  +'</div>';

  // Intelligence grid
  var iceFreshness=p.ice_fresh?'Within 6mo':p.ice_recent?'Within 1yr':p.days_since_ice<999?(Math.floor(p.days_since_ice/30)+'mo ago'):'None on record';
  var escalTxt=p.escalation>1.5?'Escalating fast':p.escalation>0.8?'Getting worse':p.escalation>0.3?'Slight trend up':'Stable';
  var factRows=[
    ...
    ['Ice Violations',p.ice_count>0?p.ice_count+'x flagged':'None',p.ice_count>0?'r':''],
    ['Last Ice Violation',iceFreshness,p.ice_fresh?'r':p.ice_recent?'o':''],
    ['Callback Inspections',p.n_callbacks>0?p.n_callbacks+'x (inspector returning)':'None',p.n_callbacks>=2?'r':p.n_callbacks===1?'o':''],
    ['Disposition Trend',escalTxt,p.escalation>1.5?'r':p.escalation>0.8?'o':'g'],
  ...
  var colorMap={r:'#dc2626',o:'#d97706',g:'#059669',b:'#2563eb'};
  var intelH='<div style="margin-bottom:12px">'
    +'<div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px">Intelligence</div>'
    +'<div style="background:#f8fafc;border-radius:8px;overflow:hidden;border:1px solid #e2e8f0">'
    +factRows.map(function(r,i){
      return '<div style="display:flex;justify-content:space-between;padding:5px 10px;'+(i%2===0?'background:#f8fafc':'background:#fff')+'">'
        +'<div style="font-size:10px;color:#64748b">'+r[0]+'</div>'
        +'<div style="font-size:10px;font-weight:600;color:'+(colorMap[r[2]]||'#1e293b')+'">'+r[1]+'</div>'
        +'</div>';
    }).join('')
    +'</div></div>';
  ...
  var histH='';
  if(entries.length){
    histH='<div style="margin-bottom:12px">'
      +'<div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px">Call History</div>'
      +[...entries].reverse().slice(0,6).map(function(e){
        var ec=OI_COLOR[normO(e.outcome)]||'#64748b';
        return '<div style="padding:5px 0;border-bottom:1px solid #f1f5f9;font-size:10px">'
          +'<span style="color:'+ec+';font-weight:700">'+(OI[normO(e.outcome)]||e.outcome)+'</span>'
          +' <span style="color:#94a3b8">'+e.date+'</span>'
          +(e.notes?'<div style="color:#64748b;font-style:italic;font-size:9px">'+e.notes+'</div>':'')
         +'</div>';
      }).join('')
    +'</div>';
  }

  // Outcome buttons — using data attrs so no inline quotes
  var outcomes=[
    {l:'\u2705 Signed',     o:'signed',     bg:'#ecfdf5',cl:'#059669',br:'#6ee7b7'},
    {l:'📅 Intro Set', o:'intro_set', bg:'#eff6ff',cl:'#2563eb',br:'#93c5fd'},
    {l:'🟡 In Play',   o:'in_play',   bg:'#fffbeb',cl:'#d97706',br:'#fcd34d'},
    {l:'🚪 No Contact',o:'no_contact',bg:'#f5f3ff',cl:'#7c3aed',br:'#c4b5fd'},
    {l:'\u274C Not Now',      o:'not_now',   bg:'#fef2f2',cl:'#dc2626',br:'#fca5a5'},
    {l:'📲 Voicemail', o:'voicemail', bg:'#fff7ed',cl:'#ea580c',br:'#fdba74'},
    {l:'\u26AB Dead',         o:'dead',      bg:'#f1f5f9',cl:'#475569',br:'#94a3b8'},
    {l:'🧼 Service Done',o:'service_done',bg:'#f0fdf4',cl:'#059669',br:'#86efac'},
  ];

  var outH='<div style="margin-bottom:10px">'
    +'<div style="font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Log Outcome</div>'
    +'<div id="sc-obtn-row" style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px;margin-bottom:8px">'
    +outcomes.map(function(o){
      return '<button data-scid="'+p.id+'" data-scout="'+o.o+'"'
        +' style="padding:9px 6px;border:1px solid '+o.br+';border-radius:8px;background:'+o.bg+';color:'+o.cl+';font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation;text-align:center">'+o.l+'</button>';
    }).join('')
    +'</div>'
    // Reason picker (hidden until Not Now or Dead selected)
    +'<div id="sc-reason-row" style="display:none;margin-bottom:8px">'
    +'<div style="font-size:9px;color:#94a3b8;margin-bottom:4px">Reason:</div>'
    +'<div id="sc-reason-btns" style="display:flex;flex-wrap:wrap;gap:5px"></div>'
    +'</div>'
    // Type toggle
    +'<div style="display:flex;gap:6px;margin-bottom:8px">'
    +'<button id="sc-type-call" data-sctype="call" style="flex:1;padding:7px;border:1px solid #e2e8f0;border-radius:7px;background:#0f1f38;color:#fff;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit;touch-action:manipulation">Phone Call</button>'
    ...
    +'</div>'
    // Notes + follow-up
    +'<textarea id="sc-notes" placeholder="Notes (optional)..." rows="2" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:7px;font-size:12px;font-family:inherit;resize:none;outline:none;box-sizing:border-box;margin-bottom:6px"></textarea>'
    +'<input id="sc-followup" type="date" placeholder="Follow-up date (optional)" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:7px;font-size:12px;font-family:inherit;outline:none;box-sizing:border-box;margin-bottom:10px">'
    +'<button id="sc-save-btn" style="width:100%;padding:11px;background:#059669;color:#fff;border:none;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation">Save Outcome</button>'
  +'</div>';

  // Won pricing
  var wonH='<div style="background:#ecfdf5;border:1px solid #6ee7b7;border-radius:8px;padding:10px;margin-bottom:12px">'
    +'<div style="font-size:9px;font-weight:700;color:#013a18;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Won — Pricing</div>'
    +'<div style="font-size:11px;color:#059669;font-weight:700">$'+(p.monthly||149)+'/mo</div>'
    +'<div style="font-size:10px;color:#475569">'+(p.machines<=1?'1 machine':p.machines===2?'2 machines ($149 + $89)':p.machines+' machines ($149 + $89 + $69\xd7'+(p.machines-2)+')')+'</div>'
    +'<div style="font-size:10px;color:#475569;margin-top:3px">One-time clean: $'+(p.onetime||249)+' \u2022 Intro offer: $99</div>'
  +'</div>';

  // Build overlay
  var bg=document.createElement('div');
  bg.id='sc-bg';
  bg.style.cssText='position:fixed;inset:0;z-index:500;background:rgba(15,31,56,.7);display:flex;align-items:flex-end;justify-content:center;-webkit-overflow-scrolling:touch';
  ...
  bg.innerHTML=
    ...
    +'<div style="position:sticky;top:0;background:#fff;border-bottom:1px solid #e2e8f0;padding:12px 16px;display:flex;justify-content:space-between;align-items:flex-start;z-index:10">'
      ...
      +'</div>'
      +'<button id="sc-close" style="margin-left:10px;flex-shrink:0;border:none;background:#f1f5f9;border-radius:50%;width:34px;height:34px;font-size:17px;cursor:pointer;display:flex;align-items:center;justify-content:center;touch-action:manipulation;color:#475569;font-family:inherit">\u2715</button>'
    +'</div>'
    +'<div style="padding:16px">'
      +'<div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:12px">'+chips+'</div>'
      +phoneH+iceH+intelH+pitchH+walkinH+objH+histH+outH+wonH
    +'</div>'
    +'</div>';

  document.body.appendChild(bg);

  // Wire buttons via delegation on the sheet (not inline onclick)
  var sheet=document.getElementById('sc-sheet');
  var selOutcome=null,selType='call';

  // Outcome buttons
  var obtnRow=document.getElementById('sc-obtn-row');
  obtnRow.addEventListener('touchend',function(e){
    var btn=e.target.closest('[data-scout]');
    if(!btn)return;
    e.preventDefault();
    selOutcome=btn.dataset.scout;
    obtnRow.querySelectorAll('button').forEach(function(b){b.style.opacity=b===btn?'1':'0.45';});
    // Show reason picker for not_now/dead
    var rrow=document.getElementById('sc-reason-row');
    var reasonMap={
      not_now:['Too expensive','Happy with current vendor','Call back later','Not the decision maker','Closed/closing'],
      dead:['Out of business','Owns machine','No ice machine','Wrong contact','Repeated refusals']
    };
    if(reasonMap[selOutcome]){
      document.getElementById('sc-reason-btns').innerHTML=reasonMap[selOutcome].map(function(r){
  ...
  obtnRow.addEventListener('click',function(e){
    var btn=e.target.closest('[data-scout]');
    if(!btn)return;
    selOutcome=btn.dataset.scout;
    obtnRow.querySelectorAll('button').forEach(function(b){b.style.opacity=b===btn?'1':'0.45';});
  ...
  // Type toggle
  var typeBtns=sheet.querySelectorAll('[data-sctype]');
  function wireTypeBtn(btn){
    function handler(e){
  ...
  typeBtns.forEach(wireTypeBtn);

  // Reason buttons
  document.getElementById('sc-reason-row').addEventListener('touchend',function(e){
    var btn=e.target.closest('[data-scr]');
    if(!btn)return;
    e.preventDefault();
    this.querySelectorAll('[data-scr]').forEach(function(b){b.style.background=b===btn?'#0f1f38':'#fff';b.style.color=b===btn?'#fff':'#1e293b';});
  },false);
  ...
  function doSave(e){
    ...
    var reasonEl=document.querySelector('#sc-reason-btns button[style*="0f1f38"]');
    var reason=reasonEl?reasonEl.dataset.scr:null;
...
function queueLogFull(outcome){
  // Open full detail modal with outcome pre-selected
  // User can add notes, reason, follow-up date before saving
  const p=queueProspects[queueIdx];
  if(!p){toast('No prospect');return;}
  _queueAutoAdvance=true;
  showCard(p.id);
  // Pre-select the outcome after modal opens
  setTimeout(()=>{
...
function saveGoals(){
  ...
  if(g('goal-clients'))goals.clients   = g('goal-clients');
  if(gs('goal-deadline'))goals.deadline= gs('goal-deadline');
  if(g('goal-mrr'))goals.mrr           = g('goal-mrr');
  if(g('goal-calls'))goals.calls       = g('goal-calls');
  if(g('goal-walkins'))goals.walkins   = g('goal-walkins');
  goalsSave();
  renderBriefing();
  toast('\u2713 Goals saved');
}
...
function openServiceLog(id){
  ...
  bg.innerHTML='<div style="background:var(--surf);border-radius:16px 16px 0 0;padding:16px;width:100%;max-width:480px;max-height:85vh;overflow-y:auto">'
    ...
    +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-bottom:10px">'
      +'<input id="svc-machine-brand" type="text" placeholder="Brand (Manitowoc, Hoshizaki...)"'
      ...
      +'<input id="svc-machine-model" type="text" placeholder="Model #"'
        +' style="padding:7px;border:1px solid var(--brd);border-radius:7px;font-size:11px;font-family:inherit;background:var(--surf);color:var(--txt);outline:none">'
      +'<input id="svc-machine-serial" type="text" placeholder="Serial #"'
        +' style="padding:7px;border:1px solid var(--brd);border-radius:7px;font-size:11px;font-family:inherit;background:var(--surf);color:var(--txt);outline:none">'
      +'<input id="svc-units" type="number" min="1" max="10" placeholder="# units" value="1"'
        +' style="padding:7px;border:1px solid var(--brd);border-radius:7px;font-size:11px;font-family:inherit;background:var(--surf);color:var(--txt);outline:none">'
    +'</div>'
...
function loadReportClient(){
  ...
  const atpPre=lastSvc?.atp_pre||c.atp_pre||'—';
  const atpPost=lastSvc?.atp_post||c.atp_post||'—';
  const hasATP=(atpPre!=='—'||atpPost!=='—');

  const today=new Date();
  const dateStr=today.toLocaleDateString('en-US',{weekday:'long',year:'numeric',month:'long',day:'numeric'});
  const nextSvc=new Date(today.getTime()+60*864e5).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'});
