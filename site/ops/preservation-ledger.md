...
| A2 `/` | **YES — fetched live 2026-07-04** | full page | **Live values (record for the rewrite decision):** title "Ice Machine Cleaning Pinellas County \| Pinellas Ice Co.", meta "Professional ice machine cleaning in Pinellas County. Remove buildup, improve performance, and keep ice safe. Same-day service available.", H1 "Is Your Ice Machine Actually Safe for Customers?" — cleaning-only framing. Rebuilt repair-first per the site-purpose directive (pre-approved: preservation does not apply to homepage emphasis). Risk note: the homepage's 385 impressions are cleaning-query impressions; the ported near-me, pricing, city, and guide pages now target those queries directly, which is the intended landing improvement. Watch homepage queries in GSC week 1 — if cleaning queries drop without the cleaning pages picking them up, revisit. |
