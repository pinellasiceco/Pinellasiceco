...
<div id="app">
  ...
  <nav class="tabs" id="main-nav">
    <div class="tab on"  onclick="sw('today')"    ><span class="tab-icon">&#x1F4CB;</span><span class="tab-lbl">Today</span></div>
    <div class="tab"     onclick="sw('all')"       ><span class="tab-icon">&#x1F4CD;</span><span class="tab-lbl">All</span></div>
    ...
    <div class="tab"     onclick="sw('pipe')"      ><span class="tab-icon">&#x1F4CA;</span><span class="tab-lbl">Pipeline</span></div>
    <div class="tab"     onclick="sw('data')"      ><span class="tab-icon">&#x2699;&#xFE0F;</span><span class="tab-lbl">Data</span></div>
  </nav>
...
const DEEP_CLEAN = {
  'Manitowoc': {
    ...
    steps: [
      ...
      {
        title: 'Identify Your Components',
        detail: 'Open the front panel (usually 2-4 screws or a lift-and-pull door). You will see: (1) The EVAPORATOR — a metal plate or set of plates with refrigerant tubing running through it, located at the top/back of the machine. This is where ice forms. Healthy evaporator plates are shiny silver/chrome. Scale appears as dull white or gray crust. Biofilm appears as pink, orange, or brown slime. (2) The WATER CURTAIN — a clear or white plastic flap hanging in front of the evaporator. It controls when ice releases. (3) The WATER DISTRIBUTION TUBE — a horizontal tube above the evaporator with small holes that spray water evenly across the plates. (4) The WATER TROUGH — the shallow reservoir at the bottom that holds and recirculates water. (5) The FLOAT VALVE — a small ball or arm in the trough that controls water level, like a toilet float. Take a photo before disassembly.',
      },
      {
  ...
  'Hoshizaki': {
    ...
    steps: [
      ...
      {
        title: 'Identify Hoshizaki-Specific Components',
        detail: 'Hoshizaki crescent cube machines look different inside than Manitowoc. Open the front panel. You will see: (1) EVAPORATOR ASSEMBLY — on Hoshizaki this is a curved/cylindrical assembly with individual crescent-shaped cube molds. The surface should be uniformly silver. Discoloration (brown, orange, dark spots) indicates mineral deposits or biofilm. (2) SPRAY BAR — a horizontal tube with small holes that sprays water over the evaporator. This is the most common maintenance issue on Hoshizaki — holes get blocked and cause uneven ice. (3) WATER TROUGH — shallower than Manitowoc, at the base. (4) FLOAT SWITCH — a small cylindrical float on a wire that controls water level. Scale on the float switch is a common cause of overflow problems. (5) CURTAIN ASSEMBLY — typically clips onto the front of the evaporator. Take a photo before disassembly.',
      },
      {
  ...
  'Scotsman': {
    ...
    steps: [
      ...
      {
        title: 'Identify Scotsman Components',
        detail: 'Open front panel. Scotsman Prodigy uses a VERTICAL evaporator — ice forms on a flat vertical plate rather than hanging from a horizontal plate. You will see: (1) EVAPORATOR PLATE — large flat metal surface on the back wall. Healthy surface is shiny. Scale appears as white/gray mineral crust. Biofilm as pink/orange slime. (2) ICE THICKNESS SENSOR — a small black probe (about the size of a pencil eraser) near the evaporator. Gap between probe tip and evaporator should be approximately 3/8 inch (the thickness of a pencil). Scale buildup on this sensor is the most common cause of Scotsman harvest failures. (3) WATER TROUGH — at the base. (4) WATER CURTAIN — plastic panel in front of evaporator. (5) DRAIN PAN — Scotsman units are notorious for drain line blockage.',
      },
      {
        title: 'Disassemble Components',
  ...
  'Ice-O-Matic': {
    ...
    steps: [
      ...
      {
        title: 'Identify Ice-O-Matic Components',
        detail: 'Open front panel. IOM units look similar to Manitowoc internally. Key components: (1) EVAPORATOR — horizontal plate at top. IOM evaporators are nickel-plated and compatible with Nickel-Safe cleaner. Scale appears white/gray. (2) WATER DISTRIBUTION PAN — sits above evaporator, has small holes that drip water evenly. The holes are the most common IOM maintenance issue — they block with mineral deposits and are smaller than most other brands. (3) WATER TROUGH — base reservoir. (4) BIN BAFFLE — a plastic divider in the bin that guides ice. Often develops biofilm on the underside. (5) WATER CURTAIN — hangs in front of evaporator.',
      },
      {
        title: 'Disassemble Components',
  ...
  'Follett': {
    ...
    steps: [
      ...
      {
        title: 'Identify Follett-Specific Components',
        detail: 'Remove top panel (typically 4 screws) to access the evaporator assembly. You will see: (1) EVAPORATOR CYLINDER — a vertical stainless steel tube, typically 6-8 inches diameter. Ice forms on the inside wall of this cylinder. Healthy cylinder interior is smooth stainless. Scale appears as white rough deposits. (2) AUGER — a helical screw inside the cylinder. Drives ice upward and out. Should spin freely. Scale on auger causes grinding and reduces ice output. (3) AUGER MOTOR — at top of cylinder. (4) ICE CHUTE — the channel ice travels through to the storage bin. On Symphony units there is also a (5) DISPENSER MECHANISM — ice chute, actuator, dispenser nozzle.',
      },
      {
  ...
  'Cornelius': {
    ...
    steps: [
      ...
      {
        title: 'Identify Cornelius Components',
        detail: 'Remove front panel (usually 2-4 screws or a clip-off panel). Cornelius layout: (1) FREEZE PLATE — a flat evaporator surface, simpler design than modular machines. Scale appears as white mineral crust. Healthy surface is smooth and shiny. (2) WATER INLET AND DISTRIBUTION — simpler nozzle system than larger brands. (3) WATER TROUGH — typically shallower on compact units. (4) DRAIN PAN — Cornelius countertop units commonly develop odor in the drain pan between services. (5) AIR FILTER — most commonly on the rear or side of the unit. This is the most frequently neglected component and the most common cause of Cornelius performance issues. Take a photo before disassembly.',
      },
      {
...
function cardHTML(p){
  ...
  const phH=p.phone
    ...
    :('<div class="phrow"><span class="phnum none">No phone on file</span><a href="https://www.google.com/search?q='+enc(p.name+' '+p.city+' FL phone number')+'" target="_blank" class="abtn find-a" onclick="event.stopPropagation()">Find</a></div>'+'<div id="ph-save-'+p.id+'" class="ph-save-row" style="margin-top:4px;display:flex;gap:4px;align-items:center">'  +'<input id="ph-inp-'+p.id+'" type="tel" placeholder="Paste number here..." onclick="event.stopPropagation()" '    +'style="flex:1;padding:5px 7px;border:1px solid var(--brd);border-radius:6px;font-size:11px;font-family:inherit;background:var(--surf);color:var(--txt);outline:none">'  +'<button onclick="event.stopPropagation();saveFoundPhone('+p.id+')" '    +'style="padding:5px 9px;border:none;border-radius:6px;background:var(--navy);color:#fff;font-size:10px;font-weight:700;cursor:pointer;font-family:inherit">Save</button>'+'</div>');
...
function saveFoundPhone(id){
  ...
  // Normalize: strip non-digits except +
  const digits=v.replace(/[^\d+\-().\s]/g,'').trim();
  PHONES[String(id)]=digits;
  phSave();
  ...
  if(vids.deep_url||vids.guide_url){
    html+='<div style="background:#fff7f5;border:1px solid #fed7aa;border-radius:8px;padding:10px;margin-bottom:10px">'
      ...
      +(vids.deep_url?'<a href="'+vids.deep_url+'" target="_blank" style="display:flex;align-items:center;gap:6px;padding:6px 8px;background:#fff;border:1px solid #fed7aa;border-radius:6px;text-decoration:none;margin-bottom:5px">'
        ...
        +'<div><div style="font-size:10px;font-weight:600;color:#ea580c">Watch: '+brand+' Cleaning Tutorial</div>'
        +'<div style="font-size:9px;color:var(--sub)">Official cleaning procedure</div></div>'
      +'</a>':'')
      +(vids.guide_url&&vids.guide_url!==vids.deep_url?'<a href="'+vids.guide_url+'" target="_blank" style="display:flex;align-items:center;gap:6px;padding:6px 8px;background:#fff;border:1px solid #fed7aa;border-radius:6px;text-decoration:none;margin-bottom:5px">'
        +'<span style="font-size:14px">&#x1F4D6;</span>'
        +'<div><div style="font-size:10px;font-weight:600;color:#ea580c">Read: Step-by-Step Guide</div>'
        +'<div style="font-size:9px;color:var(--sub)">Parts Town / Easy Ice reference</div></div>'
      +'</a>':'')
      +(vids.official_url&&vids.official_url!==vids.deep_url&&vids.official_url!==vids.guide_url?'<a href="'+vids.official_url+'" target="_blank" style="display:flex;align-items:center;gap:6px;padding:6px 8px;background:#fff;border:1px solid #fed7aa;border-radius:6px;text-decoration:none">'
        +'<span style="font-size:14px">&#x1F3ED;</span>'
