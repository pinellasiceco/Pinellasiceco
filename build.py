...
<div id="app">
  ...
  <div class="panel" id="p-service">
    ...
    <div style="display:flex;gap:0;margin-bottom:12px;border:1px solid var(--brd);border-radius:8px;overflow:hidden">
      <button class="svc-tab on" onclick="setSvcTab('cal')"   id="svct-cal">&#x1F4C5; Calendar</button>
      <button class="svc-tab"    onclick="setSvcTab('route')"  id="svct-route">&#x1F5FA; Service Route</button>
...
function setSvcTab(t){
  ...
  if(btn)btn.classList.add('on');
  ['cal','route','reports','refs'].forEach(s=>{
...
function renderServiceCal(){
  ...
  el.innerHTML=withDates.map(p=>{
    ...
    return '<div class="svc-card '+cls+'">'
      ...
      // Actions
      +'<div style="display:flex;gap:5px">'
        +'<button onclick="logServiceFromCal('+p.id+')" style="flex:2;padding:7px;border:none;border-radius:7px;background:#059669;color:#fff;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">&#x2713; Log Service Visit</button>'
...
function logServiceFromCal(id){
  const today=new Date();
  const todayStr=today.toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'});
  if(!customers[id])customers[id]={};
  customers[id].last_service=todayStr;
  ...
  renderForecast();
  renderBriefing();
  toast('Service logged. Next due: '+next.toLocaleDateString('en-US',{month:'short',day:'numeric'}));
}

function reschedule(id){
  const d=prompt('Set next service date (YYYY-MM-DD):',
...
function loadReportClient(){
  ...
  document.getElementById('svc-report-preview').innerHTML=
    ...
    +'<div style="margin-bottom:12px">'
      ...
      +'<textarea style="width:100%;padding:8px;border:1px solid var(--brd);border-radius:6px;font-size:11px;font-family:inherit;color:var(--txt);background:var(--surf);outline:none;resize:none" rows="3" placeholder="Machine condition, issues found, recommendations..."></textarea>'
    +'</div>'
    // Signatures
    ...
    +'<div style="display:flex;gap:6px;margin-top:8px">'
      ...
      +'<button onclick="logServiceFromCal('+id+')" style="flex:1;padding:9px;border:1px solid #059669;border-radius:8px;background:#ecfdf5;color:#059669;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">&#x2713; Log This Visit</button>'
    +'</div>';
}
