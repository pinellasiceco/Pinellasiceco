...
<div id="app">
  ...
  <div id="queue-bg" style="display:none;position:fixed;inset:0;background:var(--bg);z-index:90;flex-direction:column">
    ...
    <div style="padding:12px 16px;padding-bottom:max(12px,env(safe-area-inset-bottom));background:var(--surf);border-top:1px solid var(--brd2);display:grid;grid-template-columns:repeat(3,1fr);gap:8px;flex-shrink:0;position:relative;z-index:2" id="queue-actions">
      <button class="qbtn qbtn-green"  onclick="queueLog('intro_set')"  ontouchend="event.preventDefault();queueLog('intro_set')"  >&#x1F4C5; Intro Set</button>
      <button class="qbtn qbtn-yellow" onclick="queueLog('in_play')"   ontouchend="event.preventDefault();queueLog('in_play')"   >&#x1F7E1; In Play</button>
      <button class="qbtn qbtn-blue"   onclick="queueLog('no_contact')" ontouchend="event.preventDefault();queueLog('no_contact')" >&#x1F6AA; No Contact</button>
      <button class="qbtn qbtn-red"    onclick="queueLog('not_now')"   ontouchend="event.preventDefault();queueLog('not_now')"   >&#x274C; Not Now</button>
      <button class="qbtn qbtn-gray"   onclick="queueLog('voicemail')" ontouchend="event.preventDefault();queueLog('voicemail')" >&#x1F4F2; Voicemail</button>
      <button class="qbtn qbtn-gray"   onclick="queueLog('dead')"      ontouchend="event.preventDefault();queueLog('dead')"      >&#x26AB; Dead</button>
    </div>
  </div>
...
function cardHTML(p){
  ...
  const rtBtn=p.lat?('<button class="btn brt" class="route-btn" data-id="'+p.id+'" ontouchend="event.preventDefault();event.stopPropagation();addToRoute('+p.id+')" onclick="event.stopPropagation();addToRoute('+p.id+')">+Route</button>'):'';
  ...
  const custStatusH=(p.status&&p.status!=='prospect')
    ...
    :'';
  return '<div class="card '+p.priority+(isC(p.id)?' done':'')+'" data-id="'+p.id+'" onclick="openM('+p.id+')" ontouchend="event.preventDefault();openM('+p.id+')">'
    ...
    +lastH
    +'<div class="cacts">'
      +'<button class="btn blog" class="log-call-btn" data-id="'+p.id+'" ontouchend="event.preventDefault();event.stopPropagation();openM('+p.id+')" onclick="event.stopPropagation();openM('+p.id+')">Log Call</button>'
      +(last&&last.notes==='Skipped'
        ?'<button class="btn bskip" class="skip-btn" data-id="'+p.id+'" ontouchend="event.preventDefault();event.stopPropagation();unskip('+p.id+')" onclick="event.stopPropagation();unskip('+p.id+')" style="background:#ecfdf5;color:#059669;border-color:#6ee7b7">Unskip</button>'
        :'<button class="btn bskip" class="skip-btn" data-id="'+p.id+'" ontouchend="event.preventDefault();event.stopPropagation();skip('+p.id+')" onclick="event.stopPropagation();skip('+p.id+')">Skip</button>')
      +rtBtn
    +'</div>'
  +'</div>';
}
...
function renderQueueCard(){
  ...
  if(queueIdx>=queueProspects.length){
    wrap.innerHTML=
      ...
      +'<button onclick="exitQueueMode()" ontouchend="event.preventDefault();exitQueueMode()" '
       +'style="margin-top:16px;padding:10px 20px;background:var(--navy);color:#fff;border:none;'
       +'border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation">Done</button>'
      +'</div>';
  ...
  const lastStatus=lc?(OI[normO(lc.outcome)]||lc.outcome):'Not contacted';
  const lastColor=OI_COLOR[normO(lc?.outcome)]||'#64748b';
  ...
  let phoneHTML=phone
    ?'<a href="tel:'+phone.replace(/\D/g,'')+'" '
      +'style="display:inline-flex;align-items:center;gap:6px;background:#059669;color:#fff;'
      +'padding:10px 16px;border-radius:8px;font-size:14px;font-weight:700;text-decoration:none;'
      +'touch-action:manipulation">📞 '+phone+'</a>'
    :'<div style="display:flex;gap:6px;align-items:center">'
      +'<span style="font-size:11px;color:#999">No phone</span>'
      +'<a href="https://www.google.com/search?q='+enc(p.name+' '+p.city+' FL phone')+'" target="_blank" '
       +'style="font-size:10px;color:var(--blu);text-decoration:none">Search Google</a>'
     +'</div>';

  // Inspection badges
  const priorityColors={'CALLBACK':'#dc2626','HOT':'#f97316','WARM':'#eab308','COOL':'#64748b'};
  const pColor=priorityColors[p.priority]||'#64748b';
  const viol=p.highViol?'<span style="font-size:8px;padding:2px 6px;border-radius:4px;background:#fef2f2;color:#dc2626;font-weight:700">'+p.highViol+' violations</span>':'';
  const chronic=p.chronic?'<span style="font-size:8px;padding:2px 6px;border-radius:4px;background:#ecfdf5;color:#059669;font-weight:700">CHRONIC ICE</span>':'';

  // Days overdue
  const overdue=p.days_since>0?'<span style="font-size:9px;color:#dc2626;font-weight:600">'+p.days_since+'d overdue</span>':'';

  wrap.innerHTML=
    '<div style="background:#fff;border-radius:12px;padding:14px;box-shadow:0 2px 12px rgba(0,0,0,.12)">'

    // Header: name + Details button
    +'<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">'
      +'<div style="flex:1;min-width:0">'
        +'<div style="font-size:15px;font-weight:800;color:var(--navy);margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+p.name+'</div>'
        +'<div style="font-size:10px;color:var(--sub)">'+p.address+', '+p.city+'</div>'
      +'</div>'
      +'<button onclick="openM('+p.id+')" ontouchend="event.preventDefault();openM('+p.id+')" '
        +'style="flex-shrink:0;margin-left:8px;padding:7px 11px;border:1px solid var(--brd);border-radius:8px;'
        +'background:var(--navy);color:#fff;font-size:10px;font-weight:700;cursor:pointer;font-family:inherit;'
        +'touch-action:manipulation">📋 Full Card</button>'
    +'</div>'

    // Chips row: priority, status, violations, chronic, overdue
    +'<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:10px">'
      +'<span style="font-size:8px;padding:2px 7px;border-radius:4px;background:'+pColor+'20;color:'+pColor+';font-weight:700">'+p.priority+'</span>'
      +(lc?'<span style="font-size:8px;padding:2px 7px;border-radius:4px;font-weight:700;color:'+lastColor+';background:'+lastColor+'15">'+lastStatus+'</span>':'')
      +viol+chronic+overdue
    +'</div>'

    // Phone
    +'<div style="margin-bottom:10px">'+phoneHTML+'</div>'

    // Last note if any
    +(lc&&lc.notes?'<div style="font-size:10px;color:var(--sub);margin-bottom:8px;padding:6px 8px;background:#f5f8fa;border-radius:6px;line-height:1.5;font-style:italic">\u201c'+lc.notes+'\u201d</div>':'')

    // Quick note
    +'<textarea id="q-notes" placeholder="Quick note (optional)..." rows="2" '
      +'style="width:100%;padding:8px;border:1px solid var(--brd);border-radius:6px;'
      +'font-size:12px;font-family:inherit;resize:none;outline:none;color:#1a1a2e;background:#fff;box-sizing:border-box"></textarea>'
    +'</div>';
}
  ...
  el.innerHTML=mapPros.map(p=>{
    ...
    return '<div class="rcard'+(inR?' sel':'')+'" onclick="addToRoute('+p.id+')" style="border-left:3px solid '+col+(inR?';background:#fff7f5':'')+(isAnchor?';border-left:4px solid #7c3aed':'')+';">'
      ...
      +'<div style="display:flex;flex-direction:column;align-items:flex-end;gap:3px">'
        ...
        +'<button ontouchend="event.preventDefault();event.stopPropagation();openM('+p.id+')" onclick="event.stopPropagation();openM('+p.id+')" style="font-size:8px;padding:4px 8px;min-height:36px;border:1px solid var(--brd);border-radius:6px;background:var(--surf);color:var(--sub);cursor:pointer;font-family:inherit;touch-action:manipulation">Details</button>'
      +'</div>'
