...
dirs.append(Path(**file**).parent / ‘data’)
dirs.append(Path(**file**).parent)
for d in dirs:
...
alt = Path(**file**).parent / ‘data’ / ‘osm_phones.json’
if not alt.exists():
return {}
cache_path = alt
try:
...
+’<div style="font-size:9px;color:var(--sub)">’+p.city+’ • ‘+p.priority+’ • ‘+dL(p.days_until)+’</div>’
+chronicTxt
+’</div>’
...
marker.bindPopup(
‘<b>’+p.name+’</b><br>’+p.city+’ • ‘+p.priority+’<br>’+dL(p.days_until)+
...
return ‘<div class="hi">’+(OI[e.outcome]||e.outcome)+’<span style="color:var(--sub);font-size:9px"> · ‘+e.date+’</span>’+noteHtml+’</div>’;
}).join(’’);}
...
+’<div style="font-size:9px;color:var(--sub)">’+p.city+’, ‘+p.county+’ • ‘+dL(p.days_until)+’ • ‘+last.date+’</div>’
...
+’<div style="font-size:11px;color:#64748b">Contract: ‘+c.contract_start+’ → ‘+c.contract_renewal+’</div>’
+’</div>’
...
+‘Pinellas Ice Co • (727) 855-6873 • pinellasiceco.com • ’
...
+’<div style="font-weight:600;font-size:12px;color:var(--sub);margin-bottom:10px">’+p.name+’ • ‘+p.city+’</div>’
...
const diagrams={
‘Manitowoc’: `<div style="background:#fff;border:1px solid var(--brd);border-radius:8px;padding:10px;margin-bottom:10px"> <div style="font-size:10px;font-weight:700;color:var(--navy);margin-bottom:8px">&#x1F4CD; Component Diagram — Manitowoc Cube Ice Machine</div> <svg viewBox="0 0 320 220" style="width:100%;max-height:200px" xmlns="http://www.w3.org/2000/svg"> <!-- Machine outline --> <rect x="20" y="10" width="280" height="200" rx="8" fill="#f8fafc" stroke="#94a3b8" stroke-width="2"/> <!-- Evaporator plates --> <rect x="60" y="25" width="200" height="55" rx="4" fill="#dbeafe" stroke="#3b82f6" stroke-width="1.5"/> <text x="160" y="45" text-anchor="middle" font-size="10" font-weight="bold" fill="#1e40af">EVAPORATOR PLATES</text> <text x="160" y="60" text-anchor="middle" font-size="8" fill="#3b82f6">Where ice forms — clean &amp; sanitize thoroughly</text> <text x="160" y="73" text-anchor="middle" font-size="7" fill="#64748b">Scale = white/gray crust | Biofilm = pink/orange slime</text> <!-- Distribution tube --> <rect x="55" y="85" width="210" height="14" rx="3" fill="#fef9c3" stroke="#ca8a04" stroke-width="1.5"/> <text x="160" y="96" text-anchor="middle" font-size="9" font-weight="bold" fill="#92400e">WATER DISTRIBUTION TUBE — clear holes with toothpick</text> <!-- Water curtain --> <rect x="55" y="103" width="210" height="10" rx="2" fill="#f0fdf4" stroke="#16a34a" stroke-width="1.5"/> <text x="160" y="112" text-anchor="middle" font-size="8" fill="#15803d">WATER CURTAIN — remove &amp; soak in cleaner</text> <!-- Water trough --> <rect x="55" y="117" width="210" height="30" rx="3" fill="#fce7f3" stroke="#db2777" stroke-width="1.5"/> <text x="160" y="130" text-anchor="middle" font-size="9" font-weight="bold" fill="#9d174d">WATER TROUGH</text> <text x="160" y="142" text-anchor="middle" font-size="8" fill="#be185d">Highest ATP — biofilm source — scrub thoroughly</text> <!-- Float valve --> <circle cx="75" cy="132" r="8" fill="#fef3c7" stroke="#d97706" stroke-width="1.5"/> <text x="75" y="160" text-anchor="middle" font-size="7" fill="#92400e">FLOAT</text> <!-- Ice bin --> <rect x="55" y="153" width="210" height="47" rx="3" fill="#f1f5f9" stroke="#64748b" stroke-width="1.5"/> <text x="160" y="172" text-anchor="middle" font-size="9" font-weight="bold" fill="#334155">ICE BIN</text> <text x="160" y="185" text-anchor="middle" font-size="8" fill="#64748b">Sanitize all interior surfaces — discard first batch</text> <!-- Arrow labels --> <text x="30" y="55" font-size="7" fill="#64748b" transform="rotate(-90,30,55)">TOP</text> <text x="30" y="180" font-size="7" fill="#64748b" transform="rotate(-90,30,180)">BOTTOM</text> </svg> </div>`,
‘Hoshizaki’: `<div style="background:#fff;border:1px solid var(--brd);border-radius:8px;padding:10px;margin-bottom:10px"> <div style="font-size:10px;font-weight:700;color:var(--navy);margin-bottom:8px">&#x1F4CD; Component Diagram — Hoshizaki Crescent Cube Machine</div> <svg viewBox="0 0 320 220" style="width:100%;max-height:200px" xmlns="http://www.w3.org/2000/svg"> <rect x="20" y="10" width="280" height="200" rx="8" fill="#f8fafc" stroke="#94a3b8" stroke-width="2"/> <!-- Evaporator - stainless --> <rect x="60" y="25" width="200" height="50" rx="4" fill="#dbeafe" stroke="#3b82f6" stroke-width="1.5"/> <text x="160" y="43" text-anchor="middle" font-size="10" font-weight="bold" fill="#1e40af">EVAPORATOR (STAINLESS STEEL)</text> <text x="160" y="56" text-anchor="middle" font-size="8" fill="#3b82f6">Crescent cube molds — uniform silver when clean</text> <text x="160" y="69" text-anchor="middle" font-size="7" fill="#16a34a">Any commercial cleaner OK — soft brush only</text> <!-- Spray bar - highlighted as critical --> <rect x="55" y="80" width="210" height="16" rx="3" fill="#fef08a" stroke="#ca8a04" stroke-width="2"/> <text x="160" y="92" text-anchor="middle" font-size="9" font-weight="bold" fill="#92400e">&#x26A0; SPRAY BAR — MOST CRITICAL COMPONENT</text> <!-- Spray bar dots --> <circle cx="80" cy="88" r="2" fill="#ca8a04"/> <circle cx="100" cy="88" r="2" fill="#ca8a04"/> <circle cx="120" cy="88" r="2" fill="#ca8a04"/> <circle cx="140" cy="88" r="2" fill="#ca8a04"/> <circle cx="160" cy="88" r="2" fill="#ca8a04"/> <circle cx="180" cy="88" r="2" fill="#ca8a04"/> <circle cx="200" cy="88" r="2" fill="#ca8a04"/> <circle cx="220" cy="88" r="2" fill="#ca8a04"/> <circle cx="240" cy="88" r="2" fill="#ca8a04"/> <text x="160" y="106" text-anchor="middle" font-size="7" fill="#92400e">Clear EVERY hole with toothpick — blocked holes = irregular ice</text> <!-- Float switch --> <rect x="55" y="112" width="80" height="20" rx="3" fill="#fce7f3" stroke="#db2777" stroke-width="1.5"/> <text x="95" y="122" text-anchor="middle" font-size="8" font-weight="bold" fill="#9d174d">FLOAT SWITCH</text> <text x="95" y="130" text-anchor="middle" font-size="6" fill="#be185d">Must slide freely</text> <!-- Water trough --> <rect x="55" y="136" width="210" height="25" rx="3" fill="#fce7f3" stroke="#db2777" stroke-width="1.5"/> <text x="160" y="148" text-anchor="middle" font-size="9" font-weight="bold" fill="#9d174d">WATER TROUGH — highest ATP reading</text> <text x="160" y="158" text-anchor="middle" font-size="7" fill="#be185d">Scrub biofilm (pink/orange) — common in FL heat</text> <!-- Bin --> <rect x="55" y="165" width="210" height="40" rx="3" fill="#f1f5f9" stroke="#64748b" stroke-width="1.5"/> <text x="160" y="182" text-anchor="middle" font-size="9" font-weight="bold" fill="#334155">ICE BIN — Sanitize all interior surfaces</text> <text x="160" y="196" text-anchor="middle" font-size="7" fill="#64748b">Discard first 2 full bins after service</text> </svg> </div>`,
‘Scotsman’: `<div style="background:#fff;border:1px solid var(--brd);border-radius:8px;padding:10px;margin-bottom:10px"> <div style="font-size:10px;font-weight:700;color:var(--navy);margin-bottom:8px">&#x1F4CD; Component Diagram — Scotsman Prodigy Vertical Evaporator</div> <svg viewBox="0 0 320 220" style="width:100%;max-height:200px" xmlns="http://www.w3.org/2000/svg"> <rect x="20" y="10" width="280" height="200" rx="8" fill="#f8fafc" stroke="#94a3b8" stroke-width="2"/> <!-- Vertical evaporator plate --> <rect x="60" y="20" width="70" height="140" rx="4" fill="#dbeafe" stroke="#3b82f6" stroke-width="1.5"/> <text x="95" y="55" text-anchor="middle" font-size="9" font-weight="bold" fill="#1e40af">VERTICAL</text> <text x="95" y="68" text-anchor="middle" font-size="9" font-weight="bold" fill="#1e40af">EVAPORATOR</text> <text x="95" y="82" text-anchor="middle" font-size="7" fill="#3b82f6">Scotsman unique</text> <text x="95" y="94" text-anchor="middle" font-size="7" fill="#3b82f6">vertical design</text> <text x="95" y="108" text-anchor="middle" font-size="7" fill="#3b82f6">Ice forms on</text> <text x="95" y="120" text-anchor="middle" font-size="7" fill="#3b82f6">flat vertical</text> <text x="95" y="132" text-anchor="middle" font-size="7" fill="#3b82f6">plate surface</text> <!-- Critical: thickness sensor --> <circle cx="148" cy="90" r="10" fill="#fee2e2" stroke="#dc2626" stroke-width="2"/> <text x="148" y="94" text-anchor="middle" font-size="7" font-weight="bold" fill="#dc2626">SENS</text> <line x1="130" y1="90" x2="158" y2="90" stroke="#dc2626" stroke-width="1.5"/> <text x="210" y="75" text-anchor="middle" font-size="9" font-weight="bold" fill="#dc2626">&#x26A0; ICE THICKNESS</text> <text x="210" y="87" text-anchor="middle" font-size="9" font-weight="bold" fill="#dc2626">SENSOR</text> <text x="210" y="99" text-anchor="middle" font-size="7" fill="#dc2626">Gap = 3/8 inch</text> <text x="210" y="111" text-anchor="middle" font-size="7" fill="#dc2626">(pencil thickness)</text> <text x="210" y="123" text-anchor="middle" font-size="7" fill="#dc2626">Clean at EVERY visit</text> <!-- Control panel --> <rect x="145" y="25" width="140" height="40" rx="4" fill="#f0fdf4" stroke="#16a34a" stroke-width="1.5"/> <text x="215" y="40" text-anchor="middle" font-size="9" font-weight="bold" fill="#15803d">PRODIGY CONTROL</text> <text x="215" y="54" text-anchor="middle" font-size="7" fill="#15803d">Check error codes before cleaning</text> <!-- Drain pan critical --> <rect x="55" y="165" width="210" height="30" rx="3" fill="#fef9c3" stroke="#ca8a04" stroke-width="2"/> <text x="160" y="178" text-anchor="middle" font-size="9" font-weight="bold" fill="#92400e">DRAIN PAN — check at every visit</text> <text x="160" y="190" text-anchor="middle" font-size="7" fill="#92400e">Pour 1 cup water — should drain in under 30 seconds</text> <!-- Water trough --> <rect x="55" y="162" width="210" height="0" rx="3" fill="none"/> </svg> </div>`,
‘Ice-O-Matic’: `<div style="background:#fff;border:1px solid var(--brd);border-radius:8px;padding:10px;margin-bottom:10px"> <div style="font-size:10px;font-weight:700;color:var(--navy);margin-bottom:8px">&#x1F4CD; Component Diagram — Ice-O-Matic (similar to Manitowoc)</div> <svg viewBox="0 0 320 210" style="width:100%;max-height:190px" xmlns="http://www.w3.org/2000/svg"> <rect x="20" y="10" width="280" height="190" rx="8" fill="#f8fafc" stroke="#94a3b8" stroke-width="2"/> <rect x="55" y="22" width="210" height="45" rx="4" fill="#dbeafe" stroke="#3b82f6" stroke-width="1.5"/> <text x="160" y="40" text-anchor="middle" font-size="10" font-weight="bold" fill="#1e40af">EVAPORATOR PLATE</text> <text x="160" y="53" text-anchor="middle" font-size="8" fill="#3b82f6">Nickel-plated — scale forms in strips matching blocked holes</text> <!-- Distribution pan critical --> <rect x="55" y="72" width="210" height="20" rx="3" fill="#fef08a" stroke="#ca8a04" stroke-width="2"/> <text x="160" y="84" text-anchor="middle" font-size="9" font-weight="bold" fill="#92400e">&#x26A0; DISTRIBUTION PAN — MOST CRITICAL (IOM)</text> <!-- Hole dots --> <circle cx="80" cy="82" r="1.5" fill="#ca8a04"/> <circle cx="95" cy="82" r="1.5" fill="#ca8a04"/> <circle cx="110" cy="82" r="1.5" fill="#ca8a04"/> <circle cx="125" cy="82" r="1.5" fill="#ca8a04"/> <circle cx="140" cy="82" r="1.5" fill="#ca8a04"/> <circle cx="155" cy="82" r="1.5" fill="#ca8a04"/> <circle cx="170" cy="82" r="1.5" fill="#ca8a04"/> <circle cx="185" cy="82" r="1.5" fill="#ca8a04"/> <circle cx="200" cy="82" r="1.5" fill="#ca8a04"/> <circle cx="215" cy="82" r="1.5" fill="#ca8a04"/> <circle cx="230" cy="82" r="1.5" fill="#ca8a04"/> <text x="160" y="101" text-anchor="middle" font-size="7" fill="#92400e">Clear EVERY hole with toothpick — IOM unique critical step</text> <rect x="55" y="105" width="210" height="25" rx="3" fill="#fce7f3" stroke="#db2777" stroke-width="1.5"/> <text x="160" y="117" text-anchor="middle" font-size="9" font-weight="bold" fill="#9d174d">WATER TROUGH — scrub biofilm from corners</text> <text x="160" y="127" text-anchor="middle" font-size="7" fill="#be185d">Check drain flows freely</text> <!-- Bin baffle --> <rect x="90" y="134" width="140" height="15" rx="2" fill="#f0fdf4" stroke="#16a34a" stroke-width="1.5"/> <text x="160" y="145" text-anchor="middle" font-size="8" font-weight="bold" fill="#15803d">BIN BAFFLE — check underside for biofilm</text> <!-- Ice bin --> <rect x="55" y="153" width="210" height="40" rx="3" fill="#f1f5f9" stroke="#64748b" stroke-width="1.5"/> <text x="160" y="170" text-anchor="middle" font-size="9" font-weight="bold" fill="#334155">ICE BIN</text> <text x="160" y="184" text-anchor="middle" font-size="7" fill="#64748b">Sanitize all interior — discard first batch</text> </svg> </div>`,
‘Follett’: `<div style="background:#fff;border:1px solid var(--brd);border-radius:8px;padding:10px;margin-bottom:10px"> <div style="font-size:10px;font-weight:700;color:var(--navy);margin-bottom:8px">&#x1F4CD; Component Diagram — Follett Nugget Ice (Auger System)</div> <svg viewBox="0 0 320 220" style="width:100%;max-height:200px" xmlns="http://www.w3.org/2000/svg"> <rect x="20" y="10" width="280" height="200" rx="8" fill="#f8fafc" stroke="#94a3b8" stroke-width="2"/> <!-- Evaporator cylinder - vertical --> <rect x="120" y="20" width="80" height="120" rx="8" fill="#dbeafe" stroke="#3b82f6" stroke-width="2"/> <text x="160" y="40" text-anchor="middle" font-size="9" font-weight="bold" fill="#1e40af">EVAPORATOR</text> <text x="160" y="52" text-anchor="middle" font-size="9" font-weight="bold" fill="#1e40af">CYLINDER</text> <text x="160" y="65" text-anchor="middle" font-size="7" fill="#3b82f6">Stainless steel</text> <text x="160" y="77" text-anchor="middle" font-size="7" fill="#3b82f6">Ice forms on</text> <text x="160" y="89" text-anchor="middle" font-size="7" fill="#3b82f6">inside wall</text> <!-- Auger inside cylinder --> <line x1="160" y1="25" x2="160" y2="135" stroke="#7c3aed" stroke-width="3" stroke-dasharray="4,2"/> <text x="160" y="108" text-anchor="middle" font-size="8" font-weight="bold" fill="#7c3aed">AUGER</text> <text x="160" y="120" text-anchor="middle" font-size="7" fill="#7c3aed">Pushes ice up</text> <!-- Motor on top --> <rect x="130" y="8" width="60" height="14" rx="3" fill="#f5f3ff" stroke="#7c3aed" stroke-width="1.5"/> <text x="160" y="19" text-anchor="middle" font-size="7" font-weight="bold" fill="#6d28d9">AUGER MOTOR</text> <!-- Warning about auger --> <rect x="210" y="45" width="85" height="45" rx="4" fill="#fef2f2" stroke="#dc2626" stroke-width="1.5"/> <text x="252" y="60" text-anchor="middle" font-size="8" font-weight="bold" fill="#dc2626">&#x26A0; CAUTION</text> <text x="252" y="72" text-anchor="middle" font-size="7" fill="#dc2626">Auger is SHARP</text> <text x="252" y="83" text-anchor="middle" font-size="7" fill="#dc2626">Handle carefully</text> <!-- Ice chute --> <rect x="55" y="45" width="60" height="40" rx="4" fill="#fef9c3" stroke="#ca8a04" stroke-width="1.5"/> <text x="85" y="62" text-anchor="middle" font-size="8" font-weight="bold" fill="#92400e">ICE CHUTE</text> <text x="85" y="74" text-anchor="middle" font-size="7" fill="#92400e">Clean at</text> <text x="85" y="83" text-anchor="middle" font-size="7" fill="#92400e">every visit</text> <!-- Bin --> <rect x="55" y="148" width="210" height="52" rx="3" fill="#f1f5f9" stroke="#64748b" stroke-width="1.5"/> <text x="160" y="168" text-anchor="middle" font-size="9" font-weight="bold" fill="#334155">STORAGE BIN</text> <text x="160" y="180" text-anchor="middle" font-size="7" fill="#64748b">Listen for grinding before cleaning — indicates scale on cylinder</text> <text x="160" y="192" text-anchor="middle" font-size="7" fill="#64748b">Soft/moist nuggets = healthy | Hard nuggets = issue</text> <!-- Arrow showing auger direction --> <text x="205" y="100" font-size="8" fill="#7c3aed">&#x2191; ice flow</text> </svg> </div>`,
‘Cornelius’: `<div style="background:#fff;border:1px solid var(--brd);border-radius:8px;padding:10px;margin-bottom:10px"> <div style="font-size:10px;font-weight:700;color:var(--navy);margin-bottom:8px">&#x1F4CD; Component Diagram — Cornelius Compact Machine</div> <svg viewBox="0 0 320 190" style="width:100%;max-height:170px" xmlns="http://www.w3.org/2000/svg"> <rect x="20" y="10" width="280" height="170" rx="8" fill="#f8fafc" stroke="#94a3b8" stroke-width="2"/> <!-- Compact layout - front accessible --> <rect x="55" y="22" width="210" height="40" rx="4" fill="#dbeafe" stroke="#3b82f6" stroke-width="1.5"/> <text x="160" y="38" text-anchor="middle" font-size="10" font-weight="bold" fill="#1e40af">FREEZE PLATE</text> <text x="160" y="52" text-anchor="middle" font-size="7" fill="#3b82f6">Front-accessible — scale = white/gray | clean with soft brush</text> <!-- Air filter critical --> <rect x="55" y="67" width="90" height="30" rx="3" fill="#fef08a" stroke="#ca8a04" stroke-width="2"/> <text x="100" y="80" text-anchor="middle" font-size="9" font-weight="bold" fill="#92400e">&#x26A0; AIR FILTER</text> <text x="100" y="91" text-anchor="middle" font-size="7" fill="#92400e">#1 Maintenance Item</text> <!-- Location of filter --> <text x="100" y="109" text-anchor="middle" font-size="7" fill="#64748b">Side or rear mount</text> <text x="100" y="119" text-anchor="middle" font-size="7" fill="#64748b">Rinse + blow out</text> <!-- Drain pan odor --> <rect x="155" y="67" width="110" height="30" rx="3" fill="#fce7f3" stroke="#db2777" stroke-width="1.5"/> <text x="210" y="80" text-anchor="middle" font-size="9" font-weight="bold" fill="#9d174d">DRAIN PAN</text> <text x="210" y="91" text-anchor="middle" font-size="7" fill="#be185d">Odor source — scrub biofilm</text> <!-- Water trough --> <rect x="55" y="103" width="210" height="22" rx="3" fill="#fce7f3" stroke="#db2777" stroke-width="1.5"/> <text x="160" y="114" text-anchor="middle" font-size="9" font-weight="bold" fill="#9d174d">WATER TROUGH — check mineral deposits</text> <text x="160" y="122" text-anchor="middle" font-size="7" fill="#be185d">Cloudy ice = mineral contamination = filter issue</text> <!-- Bin --> <rect x="55" y="130" width="210" height="42" rx="3" fill="#f1f5f9" stroke="#64748b" stroke-width="1.5"/> <text x="160" y="148" text-anchor="middle" font-size="9" font-weight="bold" fill="#334155">ICE BIN — Sanitize thoroughly</text> <text x="160" y="161" text-anchor="middle" font-size="7" fill="#64748b">Simplest internal layout — faster service than modular units</text> </svg> </div>`,
};
...
+serviceRoute.length+’ stops • ~‘+totalMi.toFixed(1)+‘mi • est. ‘+Math.floor(estTime/60)+‘h ‘+Math.round(estTime%60)+‘m (45min/stop)</div>’
...
‘<div style="font-size:10px;color:#64748b">pinellasiceco.com  •  Pinellas County, FL</div>’+
‘</div>’+
‘</div>’+
...
+’<div style="font-size:9px;color:var(--sub)">’+( ROLE_LABELS[c.role]||c.role)+(c.phone?’ • ‘+c.phone:’’)+’</div>’
+’</div>’
