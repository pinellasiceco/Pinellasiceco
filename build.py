...
function sw(t){
  ...
  else if(t==='all'){const ag=document.getElementById('agrid');if(ag)delete ag._glAttached;populateCityFilter();rA();}
...
function attachGridListeners(grid){
  ...
  grid.addEventListener('touchend',function(e){
    ...
    if(btn){
      e.preventDefault();
      const id=parseInt(btn.closest('[data-id]')?.dataset.id||'0');
      if(!id)return;
