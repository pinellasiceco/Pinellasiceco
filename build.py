...
/* ===== REPORTS TAB ===== */
.rpt-header{display:flex;justify-content:space-between;align-items:center;padding:16px 16px 8px;border-bottom:1px solid var(--brd)}
.rpt-title{font-weight:700;font-size:15px;color:var(--navy)}
.rpt-periods{display:flex;gap:4px}
.rpt-period-btn{font-size:11px;padding:4px 10px;border-radius:12px;border:1px solid var(--brd);background:transparent;color:var(--sub);cursor:pointer;-webkit-tap-highlight-color:transparent;font-family:inherit}
.rpt-period-active{background:var(--navy);color:#fff;border-color:var(--navy)}
.rpt-section{padding:16px;border-bottom:1px solid var(--brd)}
.rpt-section-highlight{background:#f0fdf4;border-left:3px solid #059669}
.rpt-section-title{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:var(--sub);margin-bottom:12px}
.rpt-kpi-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-bottom:12px}
.rpt-kpi-card{background:var(--surf);border:1px solid var(--brd);border-radius:6px;padding:12px;box-shadow:var(--shadow)}
.rpt-kpi-val{font-size:22px;font-weight:800;color:var(--navy);line-height:1;margin-bottom:4px}
.rpt-kpi-label{font-size:11px;color:var(--sub);font-weight:500}
...
.funnel-row .funnel-label{font-size:11px;color:var(--sub);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.funnel-row .funnel-bar-wrap{background:var(--brd2);border-radius:2px;height:10px;overflow:hidden}
...
.rpt-table-wrap{overflow-x:auto;border:1px solid var(--brd);border-radius:4px}
.rpt-table{width:100%;border-collapse:collapse;font-size:12px}
.rpt-table th{background:var(--surf);padding:8px 10px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:0.08em;color:var(--sub);white-space:nowrap;border-bottom:1px solid var(--brd)}
.rpt-table td{padding:8px 10px;border-top:1px solid var(--brd);color:var(--txt)}
.rpt-chart-wrap{overflow-x:auto;padding:8px 0 4px;-webkit-overflow-scrolling:touch}
.rpt-sub{font-size:11px;color:var(--sub);margin-top:6px}
.rpt-empty{text-align:center;padding:60px 24px;color:var(--sub)}
.reports-loading{padding:40px 24px;text-align:center;color:var(--sub);font-size:13px}
  ...
  <header>
    <div class="logo">
      ...
      <div><div class="logo-name">Pinellas Ice Co</div>
        <div id="app-version" onclick="diagTap()" style="font-size:8px;color:var(--sub);letter-spacing:.04em;cursor:pointer;user-select:none">PROSPECT TOOL &bull; %%DATE%% &bull; v8 &bull; <span id="data-freshness"></span></div>
      </div>
    </div>
    <div class="hchips">
...
<div style="padding:0 0 8px 0;text-align:right">
      <button onclick="openAddBusinessModal()" ontouchend="event.preventDefault();openAddBusinessModal()" style="font-size:11px;font-weight:700;padding:6px 14px;border:1.5px dashed #15803d;border-radius:8px;background:#f0fdf4;color:#15803d;cursor:pointer;font-family:inherit;touch-action:manipulation">&#x2B; Add Business</button>
    </div>
<div class="grid" id="agrid"></div>
  ...
  <div class="panel" id="p-data">
    ...
    <div class="dc" style="margin-top:12px">
      ...
      <a href="/Pinellasiceco/docs/fieldmanual/" target="_blank" rel="noopener" style="display:block;width:100%;padding:9px;background:var(--navy);color:#fff;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;text-align:center;text-decoration:none;box-sizing:border-box">Open Technician Field Manual &#x2197;</a>
    </div>
    ...
    <div class="dc" style="margin-top:12px">
      ...
      <button class="xbtn" onclick="exportDirectoryData()" style="background:#059669;color:#fff;border-color:#059669;margin-bottom:6px;width:100%">&#x1F4E5; Export Directory Data</button>
      <button class="xbtn" onclick="exportCleanScoreJSON()" ontouchend="event.preventDefault();exportCleanScoreJSON()" style="background:#0f766e;color:#fff;border-color:#0f766e;margin-bottom:10px;width:100%;touch-action:manipulation">&#x1F4CB; Export CleanScore JSON</button>
...
function doResetLocal(){
  ...
  deleteAllCustomersFromSupabase(function(){
...
function cardHTML(p){
  ...
  const manualBadge=p.manual_entry?('<span style="font-size:9px;padding:1px 5px;border-radius:4px;background:#f0fdf4;color:#15803d;border:1px solid #86efac;font-weight:700;margin-left:4px">&#x270F; Added by You</span>'):'';
...
function showCard(id){
  ...
  var factRows=[
    ...
    ['Last Inspected',(p.cit_latest&&p.cit_latest>(p.last_insp||'')?p.cit_latest:p.last_insp)||'—','#1e293b'],
  ...
  var closeH=_alreadyWon
    ...
    :('<div style="margin-bottom:12px">'
      ...
      +(p.manual_entry?('<button id="sc-del-manual-btn" onclick="deleteManualBusiness(&#39;'+p.id+'&#39;)" ontouchend="event.preventDefault();deleteManualBusiness(&#39;'+p.id+'&#39;)" style="width:100%;margin-top:6px;padding:7px;border:1px solid #fecaca;border-radius:8px;background:#fef2f2;color:#dc2626;font-size:10px;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation">&#x1F5D1; Remove This Business</button>'):'')
      +'</div>');
  ...
  var contactsH='<div style="margin-bottom:12px">'
    ...
    +(c.dm_name?'<div style="font-size:11px;font-weight:600;color:#0f172a;margin-bottom:4px">'+c.dm_name+(c.dm_title?'<span style="font-size:10px;font-weight:400;color:#64748b;margin-left:4px">&middot; '+c.dm_title+'</span>':'')+'</div>':'')
    ...
    +'</div>'
    +'<div style="display:flex;gap:5px;margin-bottom:5px">'
    +'<input id="sc-dm-title" type="text" value="'+(c.dm_title||'')+'" placeholder="Title or role" '
    +'style="flex:1;padding:7px;border:1px solid #bae6fd;border-radius:6px;font-size:12px;font-family:inherit;background:#fff;color:#1e293b;outline:none">'
    +'</div>'
  ...
  function scHandle(e){
    ...
    if(bid==='sc-save-dm'){
      ...
      var dmTitle=(document.getElementById('sc-dm-title')||{}).value||'';
      ...
      customers[p.id].dm_title=dmTitle;
...
function openM(id){
  ...
  document.getElementById('mfacts').innerHTML=[
    ...
    ['Last Inspected',(p.cit_latest&&p.cit_latest>(p.last_insp||'')?p.cit_latest:p.last_insp)||'',''],
...
function buildCleanScoreExport(){
  ...
  for(var pid in customers){
    ...
    if(!certId){
      seq++;
      certId='PIC-'+String(seq).padStart(4,'0');
      c.cert_id=certId;
      changed=true;
    }
    var biz=c.name||'';
  ...
  function nullify(v){
    ...
    var r=Array.isArray(v)?[]:{};
    for(var k in v){r[k]=v[k]===undefined?null:nullify(v[k]);}
    return r;
  }
  return nullify(output);
}
function exportCleanScoreJSON(){
  var data=buildCleanScoreExport();
  var json=JSON.stringify(data,null,2);
  var blob=new Blob([json],{type:'application/json'});
  var url=URL.createObjectURL(blob);
  var a=document.createElement('a');
  a.href=url;a.download='cleanscore.json';
  document.body.appendChild(a);a.click();document.body.removeChild(a);
  setTimeout(function(){URL.revokeObjectURL(url);},1000);
  toast('cleanscore.json downloaded — commit to repo root');
}
function clrLog(){
...
function deleteAllCustomersFromSupabase(onDone){
  var key=_SUPABASE_ANON_KEY||(localStorage.getItem('pic_supabase_key')||'').trim();
  function done(){
    localStorage.removeItem('pic_supa_last_pull');
    localStorage.removeItem('pic_supa_bootstrapped');
    if(_sb&&_userId){try{_sb.from('pic_customers').delete().eq('device_id',_userId);}catch(e){}}
    if(onDone)onDone();
  }
  if(!key){done();return;}
  fetch(picSupabaseUrl()+'/customers?updated_at=gte.1970-01-01T00:00:00.000Z',{
    method:'DELETE',
    headers:{'Authorization':'Bearer '+key,'apikey':key,'Prefer':'return=minimal'}
  }).then(done).catch(done);
}
function clrCustomers(){
  if(!confirm('Clear all customer data? Clears local and cloud. Cannot be undone.'))return;
  customers={};
  try{localStorage.setItem('pic_customers','{}');}catch(e){}
  P.forEach(p=>{if(p.status!=='prospect')p.status='prospect';});
  if(tab==='clients')rCust();
  renderBriefing();
  toast('Customer data cleared');
  deleteAllCustomersFromSupabase();
}
function clrAll(){
  if(!confirm('Reset ALL data? Clears log, customers, contacts, vendors, goals and cloud customer records. Cannot be undone.'))return;
  log={};lSave();
  customers={};
  try{localStorage.setItem('pic_customers','{}');}catch(e){}
...
async function clrAllCloud(){
  ...
  vendors={};vendorsSave();
  P.forEach(p=>{p.status='prospect';});
  await new Promise(function(resolve){deleteAllCustomersFromSupabase(resolve);});
  if(_sb&&_userId){
    try{
      await Promise.all([
...
function pushCustomerToSupabase(pid){
  ...
  fetch(picSupabaseUrl()+'/customers?on_conflict=pid',{
    ...
    if(!r.ok)r.text().then(function(t){toast('Sync error '+r.status+': '+t.slice(0,60));});
  }).catch(function(){});
}
...
function markSupplementalWon(){
  ...
  var _co=document.getElementById('close-overlay');if(_co)_co.remove();
  var _mbg=document.getElementById('mbg');if(_mbg)_mbg.classList.remove('on');
  sw('customers');
}
function logSupplementalService(id){
  ...
  var _suppP=P.find(function(x){return x.id==id;});
  if(_suppP){setTimeout(function(){scSupplementalReport(_suppP,{notes:''});},300);}
}
...
function rCust(){
  ...
  // Use customers dict as authoritative status source; fall back to p.status
  ...
  try{
    ...
    if(r4.data&&r4.data.length){
      ...
      r4.data.forEach(function(row){
        ...
        if(!p&&row.data._manual_rec){
          P.push(row.data._manual_rec);p=P[P.length-1];
          try{var _ms=JSON.parse(localStorage.getItem('pic_manual_prospects')||'[]');
            if(!_ms.find(function(x){return String(x.id)===row.prospect_id;})){_ms.push(row.data._manual_rec);localStorage.setItem('pic_manual_prospects',JSON.stringify(_ms));}
          }catch(e){}
        }
        if(p){
          if(row.data.status){
...
function scSupplementalReport(p,prefill){
  var suppBg=document.createElement('div');
  suppBg.id='supp-bg';
  suppBg.style.cssText='position:fixed;inset:0;z-index:600;background:rgba(15,31,56,.88);display:flex;align-items:center;justify-content:center;padding:20px;box-sizing:border-box';
  var box=document.createElement('div');
  box.style.cssText='background:#fff;border-radius:16px;padding:24px;width:100%;max-width:340px';
  box.innerHTML=
    '<div style="font-size:14px;font-weight:800;color:#7c3aed;margin-bottom:4px">&#x1F9FC; Supplemental Service Report</div>'
    +'<div style="font-size:11px;color:#64748b;margin-bottom:16px">Add an email to send the visit record to your client, or print for your files.</div>'
    +'<input id="supp-email-inp" type="email" placeholder="Email to (optional)" '
    +'style="width:100%;padding:10px;border:2px solid #e2e8f0;border-radius:10px;font-size:14px;font-family:inherit;outline:none;box-sizing:border-box;margin-bottom:8px;color:#0f1f38">'
    +'<textarea id="supp-notes-inp" placeholder="Technician notes (optional)" rows="2" '
    +'style="width:100%;padding:10px;border:2px solid #e2e8f0;border-radius:10px;font-size:13px;font-family:inherit;outline:none;box-sizing:border-box;margin-bottom:12px;color:#0f1f38;resize:none"></textarea>'
    +'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">'
    +'<button id="supp-cancel" style="padding:10px;border:1px solid #e2e8f0;border-radius:9px;background:#f8fafc;color:#64748b;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation">Cancel</button>'
    +'<button id="supp-email" style="padding:10px;border:1px solid #7c3aed;border-radius:9px;background:#f5f3ff;color:#7c3aed;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation">&#x1F4E7; Email</button>'
    +'<button id="supp-print" style="padding:10px;border:none;border-radius:9px;background:#7c3aed;color:#fff;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation">Print</button>'
    +'</div>';
  suppBg.appendChild(box);
  ...
  function suppHandle(e){
    if(e.type==='click'&&Date.now()-_lt<350)return;
    if(e.type==='touchend')_lt=Date.now();
    if(e.target===suppBg){suppBg.remove();return;}
    var btn=e.target.closest('#supp-cancel,#supp-email,#supp-print');
    if(!btn)return;
    if(e.type==='touchend')e.preventDefault();
    if(btn.id==='supp-cancel'){suppBg.remove();return;}
    var emailTo=((document.getElementById('supp-email-inp')||{}).value||'').trim();
    var notes=((document.getElementById('supp-notes-inp')||{}).value||'').trim();
    if(btn.id==='supp-email'){
      if(!emailTo){toast('Enter an email address first');return;}
  ...
  suppBg.addEventListener('click',suppHandle);
  suppBg.addEventListener('touchend',suppHandle);
}
function srGenerateSupplemental(p,notes){
  var c=customers[p.id]||{};
  var dateStr=new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'});
  var nextRaw=c.supplemental_next_service||c.next_service;
  var nextStr=nextRaw?new Date(nextRaw.replace(/-/g,'/')).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'}):'Contact us to schedule';
  var services=['Soda lines &amp; nozzle cleaning','Exterior ice machine &amp; splash zone wipe-down','Reach-in cooler gasket &amp; seal inspection and cleaning'];
  var html='<!DOCTYPE html><html><head><meta charset="UTF-8">'
    +'<meta name="viewport" content="width=device-width,initial-scale=1">'
    +'<title>Supplemental Service Report</title>'
    +'<style>'
    +'*{box-sizing:border-box}body{margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",sans-serif;background:#fff;color:#0f172a}'
    +'@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}button{display:none}.page{zoom:0.92;padding:0}.rpt-bar{display:none}}'
    +'@page{size:letter portrait;margin:0.3in}'
    ...
    +'<div class="rpt-bar">'
    +'<button onclick="if(window.opener)window.opener.focus();window.close()" style="flex:1;padding:10px;background:#1e3a5f;color:#e2e8f0;border:1px solid #334155;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">&#x2190; Back to App</button>'
    +'<button onclick="window.print()" style="flex:1;padding:10px;background:#7c3aed;color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">&#x1F5A8; Print</button>'
    +'</div>'
    +'<div class="page">'
    +'<table style="width:100%;border-collapse:collapse;margin-bottom:20px"><tr>'
    +'<td style="vertical-align:middle"><img src="logo_for_pdf.png" onerror="this.remove()" style="height:42px;display:block"></td>'
    +'<td style="text-align:right;vertical-align:middle">'
    +'<div style="font-size:17px;font-weight:900;color:#7c3aed;letter-spacing:-.01em">SUPPLEMENTAL SERVICE REPORT</div>'
    +'<div style="font-size:11px;color:#64748b;margin-top:2px">pinellasiceco.com &nbsp;&middot;&nbsp; '+dateStr+'</div>'
    +'</td></tr></table>'
    +'<div style="border-top:2px solid #7c3aed;margin-bottom:10px"></div>'
    +'<div style="border:1px solid #e2e8f0;border-radius:8px;padding:14px;margin-bottom:10px">'
    +'<div style="font-size:8px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#64748b;margin-bottom:6px">Establishment</div>'
    +'<div style="font-size:14px;font-weight:800;color:#0f1f38;margin-bottom:4px">'+p.name+'</div>'
    +'<div style="font-size:11px;color:#475569">'+p.address+'</div>'
    +'<div style="font-size:11px;color:#475569">'+p.city+', FL '+p.zip+'</div>'
    +'</div>'
    +'<div style="border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;margin-bottom:10px">'
    +'<div style="background:#7c3aed;padding:10px 14px;font-size:8px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#f5f3ff">Services Performed This Visit</div>'
    +'<div style="padding:12px 14px">'
    +services.map(function(s){return '<div style="font-size:11px;color:#334155;padding:3px 0;display:flex;gap:8px"><span style="color:#7c3aed;font-weight:700;flex-shrink:0">&#x2713;</span><span>'+s+'</span></div>';}).join('')
    +'</div></div>'
    +'<div style="background:#0f1f38;border-radius:10px;padding:12px;text-align:center;margin-bottom:10px">'
    +'<div style="font-size:8px;color:#94a3b8;letter-spacing:.12em;text-transform:uppercase;margin-bottom:6px">Next Scheduled Service</div>'
    +'<div style="font-size:22px;font-weight:900;color:#f97316;margin-bottom:4px">'+nextStr+'</div>'
    +'<div style="font-size:11px;color:#e2e8f0;margin-bottom:8px">Thank you for choosing Pinellas Ice Co</div>'
    +'<div style="font-size:16px;font-weight:800;color:#fff">Call&nbsp;/&nbsp;Text:&nbsp;&nbsp;(727)&nbsp;855-6873</div>'
    +'<div style="font-size:10px;color:#94a3b8;margin-top:4px">pinellasiceco.com</div>'
    +'</div>'
    +(notes?('<div style="border:1px solid #e2e8f0;border-radius:8px;padding:14px;margin-bottom:10px">'
      ...
      +'</div>'):'')
    +'<div style="text-align:center;font-size:9px;color:#94a3b8;border-top:1px solid #e2e8f0;padding-top:10px;margin-top:14px">'
    +'Pinellas Ice Co &nbsp;&middot;&nbsp; pinellasiceco.com &nbsp;&middot;&nbsp; (727) 855-6873'
    +'</div>'
    +'</div></body></html>';
  var w=window.open('','_blank');
  if(!w){toast('Pop-up blocked — allow pop-ups and try again');return;}
...
function srSendEmailSupplemental(p,emailTo,notes){
  var c=customers[p.id]||{};
  var now=new Date();
  var dateStr=now.toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'});
  var nextRaw=c.supplemental_next_service||c.next_service;
  ...
  var html='<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>'
    ...
    +'<td align="right"><div style="font-size:11px;color:#64748b">'+dateStr+'</div></td>'
    +'</tr></table>'
    +'<div style="font-size:15px;font-weight:800;color:#7c3aed;margin-bottom:10px">SUPPLEMENTAL SERVICE REPORT</div>'
    +'<div style="border-top:2px solid #7c3aed;margin-bottom:14px"></div>'
    ...
    +'<div style="background:#7c3aed;padding:10px 14px;font-size:8px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#f5f3ff">Services Performed This Visit</div>'
    +'<div style="padding:12px 14px">'
    +services.map(function(s){return '<div style="font-size:11px;color:#334155;padding:3px 0"><span style="color:#7c3aed;font-weight:700">&#x2713;</span>&nbsp;&nbsp;'+s+'</div>';}).join('')
    +'</div></div>'
    +'<div style="background:#0f1f38;border-radius:10px;padding:14px;text-align:center;margin-bottom:14px">'
    +'<div style="font-size:8px;color:#94a3b8;letter-spacing:.12em;text-transform:uppercase;margin-bottom:6px">Next Scheduled Service</div>'
    +'<div style="font-size:18px;font-weight:900;color:#f97316;margin-bottom:4px">'+nextStr+'</div>'
    +'<div style="font-size:11px;color:#e2e8f0;margin-bottom:8px">Thank you for choosing Pinellas Ice Co</div>'
    +'<div style="font-size:16px;font-weight:800;color:#fff">Call&nbsp;/&nbsp;Text:&nbsp;&nbsp;(727)&nbsp;855-6873</div>'
    +'<div style="font-size:10px;color:#94a3b8;margin-top:4px">pinellasiceco.com</div>'
    +'</div>'
    ...
    +'</div>'
    +'</div></body></html>';
  return sendEmailViaProxy(emailTo,'Supplemental Service Report — '+p.name,html);
}
...
function updateSyncDot(mode){
  var dot=document.getElementById('sync-dot');
  if(!dot)return;
  if(mode==='synced'){dot.style.background='#059669';dot.title='Live sync active';_syncErrors=0;return;}
  if(mode==='local'){dot.style.background='#d97706';dot.title='Reconnecting...';return;}
  if(mode==='error'){dot.style.background='#dc2626';dot.title=_syncErrors+' sync error(s) — tap to diagnose';return;}
  ...
  if(_userId&&_syncErrors>0){dot.style.background='#dc2626';dot.title=_syncErrors+' sync errors — tap to diagnose';}
  else if(_userId){dot.style.background='#059669';dot.title='Logged in — cloud sync active';}
...
function diagTap(){
  ...
  if(_diagTaps>=3){_diagTaps=0;showDiagnostics();return;}
  _diagTapTimer=setTimeout(function(){_diagTaps=0;},800);
}

async function showDiagnostics(){
  var uid=_userId||localStorage.getItem('pic_device_id')||'(none)';
  var custCount=Object.keys(customers).length;
  var logCount=Object.keys(log).length;
  var lines=['=== PIC Diagnostics v8 ===',
    'Device ID: '+uid,
    'Session: '+(_session?'active':'none'),
    'Sync errors: '+_syncErrors,
    'Last OK: '+(_lastSyncOk?_lastSyncOk.toLocaleTimeString():'never'),
    'Customers in RAM: '+custCount,
    'Log entries: '+logCount,
    'Prospects loaded: '+P.length,
    ''];
  if(_sb&&_userId){
...
async function sbHealthCheck(){
  ...
  if(w.error){
    _syncErrors++;
    updateSyncDot('error');
    toast('⚠ Supabase write test failed — check connection. Error: '+w.error.message.slice(0,80));
    console.error('sbHealthCheck write failed:',w.error);
    return;
  }
  // Test read-back
  ...
  if(r.error||!r.data){
    _syncErrors++;
    updateSyncDot('error');
    toast('⚠ Supabase read-back failed — data may not be persisting');
    console.error('sbHealthCheck readback failed:',r.error);
    return;
  }
  _lastSyncOk=new Date();
  console.log('sbHealthCheck: OK — writes and reads working');
}
...
function mergeManualProspects(){
  try{
    var stored=localStorage.getItem('pic_manual_prospects');
    if(!stored)return;
    var manual=JSON.parse(stored);
    if(!Array.isArray(manual)||!manual.length)return;
    var existingIds=new Set(P.map(function(x){return String(x.id);}));
    manual.forEach(function(r){if(!existingIds.has(String(r.id))){P.push(r);}});
  }catch(e){console.warn('mergeManualProspects error:',e);}
}
function openAddBusinessModal(){
  var existing=document.getElementById('add-biz-bg');
  if(existing)existing.remove();
  var bg=document.createElement('div');
  bg.id='add-biz-bg';
  bg.style.cssText='position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.55);z-index:1000;overflow-y:auto;-webkit-overflow-scrolling:touch;display:flex;align-items:flex-start;justify-content:center;padding:16px;box-sizing:border-box;';
  bg.innerHTML='<div style="background:#fff;border-radius:14px;padding:20px;width:100%;max-width:480px;margin-top:20px;box-sizing:border-box;">'
    +'<div style="font-size:17px;font-weight:700;color:#0f1f38;margin-bottom:16px;display:flex;justify-content:space-between;align-items:center;">'
    +'Add Business'
    +'<button onclick="document.getElementById(&#39;add-biz-bg&#39;).remove()" ontouchend="event.preventDefault();document.getElementById(&#39;add-biz-bg&#39;).remove()" style="background:none;border:none;font-size:22px;color:#64748b;cursor:pointer;padding:0 4px;touch-action:manipulation;">&#x2715;</button>'
    +'</div>'
    +'<div id="add-biz-err" style="display:none;background:#fef2f2;border:1px solid #fecaca;border-radius:7px;padding:8px 12px;font-size:12px;color:#dc2626;margin-bottom:12px;"></div>'
    +'<div style="margin-bottom:12px;">'
    +'<label style="font-size:11px;font-weight:600;color:#475569;display:block;margin-bottom:4px;">BUSINESS NAME *</label>'
    +'<input id="abiz-name" type="text" placeholder="e.g. The Rusty Anchor Bar" style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:8px;font-size:14px;font-family:inherit;box-sizing:border-box;outline:none;">'
    +'</div>'
    +'<div style="margin-bottom:12px;">'
    +'<label style="font-size:11px;font-weight:600;color:#475569;display:block;margin-bottom:4px;">ADDRESS *</label>'
    +'<input id="abiz-addr" type="text" placeholder="123 Main St" style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:8px;font-size:14px;font-family:inherit;box-sizing:border-box;outline:none;">'
    +'</div>'
    +'<div style="display:flex;gap:10px;margin-bottom:12px;">'
    +'<div style="flex:2"><label style="font-size:11px;font-weight:600;color:#475569;display:block;margin-bottom:4px;">CITY *</label>'
    +'<input id="abiz-city" type="text" placeholder="Clearwater" style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:8px;font-size:14px;font-family:inherit;box-sizing:border-box;outline:none;"></div>'
    +'<div style="flex:1"><label style="font-size:11px;font-weight:600;color:#475569;display:block;margin-bottom:4px;">ZIP *</label>'
    +'<input id="abiz-zip" type="text" placeholder="33755" maxlength="5" style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:8px;font-size:14px;font-family:inherit;box-sizing:border-box;outline:none;"></div>'
    +'</div>'
    +'<div style="margin-bottom:12px;">'
    +'<label style="font-size:11px;font-weight:600;color:#475569;display:block;margin-bottom:4px;">PHONE</label>'
    +'<input id="abiz-phone" type="tel" placeholder="(727) 555-1234" style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:8px;font-size:14px;font-family:inherit;box-sizing:border-box;outline:none;">'
    +'</div>'
    +'<div style="margin-bottom:12px;">'
    +'<label style="font-size:11px;font-weight:600;color:#475569;display:block;margin-bottom:4px;">VENUE TYPE</label>'
    +'<select id="abiz-vtype" style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:8px;font-size:14px;font-family:inherit;box-sizing:border-box;outline:none;background:#fff;">'
    +'<option value="">-- select --</option>'
    +'<option value="restaurant">Restaurant</option>'
    +'<option value="bar">Bar</option>'
    +'<option value="hotel">Hotel</option>'
    +'<option value="golf">Golf / Country Club</option>'
    +'<option value="cafe">Cafe</option>'
    +'<option value="other">Other</option>'
    +'</select>'
    +'</div>'
    +'<div style="display:flex;gap:10px;margin-bottom:12px;">'
    +'<div style="flex:1"><label style="font-size:11px;font-weight:600;color:#475569;display:block;margin-bottom:4px;">LATITUDE (optional)</label>'
    +'<input id="abiz-lat" type="number" step="any" placeholder="27.9758" style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:8px;font-size:14px;font-family:inherit;box-sizing:border-box;outline:none;"></div>'
    +'<div style="flex:1"><label style="font-size:11px;font-weight:600;color:#475569;display:block;margin-bottom:4px;">LONGITUDE (optional)</label>'
    +'<input id="abiz-lon" type="number" step="any" placeholder="-82.8001" style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:8px;font-size:14px;font-family:inherit;box-sizing:border-box;outline:none;"></div>'
    +'</div>'
    +'<div style="margin-bottom:20px;">'
    +'<label style="font-size:11px;font-weight:600;color:#475569;display:block;margin-bottom:4px;">NOTES</label>'
    +'<textarea id="abiz-notes" rows="3" placeholder="Saw 2 machines, owner was friendly..." style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:8px;font-size:14px;font-family:inherit;box-sizing:border-box;outline:none;resize:vertical;"></textarea>'
    +'</div>'
    +'<div style="display:flex;gap:10px;">'
    +'<button onclick="document.getElementById(&#39;add-biz-bg&#39;).remove()" ontouchend="event.preventDefault();document.getElementById(&#39;add-biz-bg&#39;).remove()" style="flex:1;padding:13px;border:1px solid #e2e8f0;border-radius:10px;font-size:15px;font-weight:600;color:#475569;background:#fff;cursor:pointer;font-family:inherit;touch-action:manipulation;">Cancel</button>'
    +'<button onclick="saveNewBusiness()" ontouchend="event.preventDefault();saveNewBusiness()" style="flex:2;padding:13px;background:#0f1f38;color:#fff;border:none;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation;">Save Business</button>'
    +'</div>'
    +'</div>';
  document.body.appendChild(bg);
  setTimeout(function(){var el=document.getElementById('abiz-name');if(el)el.focus();},100);
}
function saveNewBusiness(){
  var name=(document.getElementById('abiz-name').value||'').trim();
  var addr=(document.getElementById('abiz-addr').value||'').trim();
  var city=(document.getElementById('abiz-city').value||'').trim();
  var zip=(document.getElementById('abiz-zip').value||'').trim();
  var phone=(document.getElementById('abiz-phone').value||'').trim();
  var vtype=(document.getElementById('abiz-vtype').value||'').trim();
  var latRaw=(document.getElementById('abiz-lat').value||'').trim();
  var lonRaw=(document.getElementById('abiz-lon').value||'').trim();
  var notes=(document.getElementById('abiz-notes').value||'').trim();
  var errEl=document.getElementById('add-biz-err');
  if(!name||!addr||!city||!zip){errEl.textContent='Please fill in all required fields (name, address, city, zip).';errEl.style.display='block';return;}
  errEl.style.display='none';
  var lat=latRaw?parseFloat(latRaw):0;
  var lon=lonRaw?parseFloat(lonRaw):0;
  var newId='MANUAL-'+Date.now();
  var rec={id:newId,name:name,county:'Pinellas',city:city,address:addr,zip:zip,lat:lat,lon:lon,
    last_insp:'',last_disp:'',disp_risk:0,high_viol:0,total_viol:0,n_insp:0,pred_next:'',days_until:0,days_since:0,
    pitch_type:'',score:0,confidence:0,priority:'WARM',ice_rel:0,confirmed:false,chronic:false,ice_count:0,codes:[],
    trending:false,is_emergency:false,biz_type:'independent',is_bar:vtype==='bar',venue_type:vtype||'restaurant',
    seats:0,machines:1,monthly:149,quarterly:129,onetime:395,intro:99,year1_monthly:1887,year1_quarterly:1647,
    tier:'',phone:phone,status:'prospect',rating:0,build_date:'',hours:'',n_callbacks:0,escalation:0,
    ice_recent:false,ice_fresh:false,days_since_ice:0,code_diversity:0,avg_visit:0,ice_risk_prob:0,
    ice_risk_level:'',ice_risk_reason:'',new_reason:null,change_severity:null,cit_count:0,cit_ice_count:0,
    cit_latest:'',cit_earliest:'',cit_days_since:0,cit_observation:'',cit_observation_date:'',cit_codes:[],
    cit_repeat:0,cit_mold_black:0,cit_mold_pink:0,cit_scoop:0,ice_confirmed_dbpr:false,ice_gold:false,
    premium_score:0,manual_entry:true};
  if(!customers[newId])customers[newId]={};customers[newId].notes=notes;customers[newId]._manual_rec=rec;custSave();
  var stored=[];try{stored=JSON.parse(localStorage.getItem('pic_manual_prospects')||'[]');}catch(e){}
  stored.push(rec);
  localStorage.setItem('pic_manual_prospects',JSON.stringify(stored));
  P.push(rec);
  var bg=document.getElementById('add-biz-bg');if(bg)bg.remove();
  rA();
  toast('Business added &#8212; ready to log');
}
function deleteManualBusiness(id){
  if(!confirm('Remove this business from your list?'))return;
  try{var stored=JSON.parse(localStorage.getItem('pic_manual_prospects')||'[]');stored=stored.filter(function(r){return String(r.id)!==String(id);});localStorage.setItem('pic_manual_prospects',JSON.stringify(stored));}catch(e){}
  for(var i=0;i<P.length;i++){if(String(P[i].id)===String(id)){P.splice(i,1);break;}}
