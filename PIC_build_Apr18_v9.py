...
<div id="app">
  ...
  <div id="queue-bg" style="display:none;position:fixed;inset:0;background:var(--bg);z-index:90;flex-direction:column">
    ...
    <div style="padding:12px 16px;padding-bottom:max(12px,env(safe-area-inset-bottom));background:var(--surf);border-top:1px solid var(--brd2);display:grid;grid-template-columns:repeat(3,1fr);gap:8px;flex-shrink:0;position:relative;z-index:2" id="queue-actions">
      <button class="qbtn qbtn-green"  data-action="intro_set"  >&#x1F4C5; Intro Set</button>
      <button class="qbtn qbtn-yellow" data-action="in_play"    >&#x1F7E1; In Play</button>
      <button class="qbtn qbtn-blue"   data-action="no_contact" >&#x1F6AA; No Contact</button>
      <button class="qbtn qbtn-red"    data-action="not_now"    >&#x274C; Not Now</button>
      <button class="qbtn qbtn-gray"   data-action="voicemail"  >&#x1F4F2; Voicemail</button>
      <button class="qbtn qbtn-gray"   data-action="dead"       >&#x26AB; Dead</button>
    </div>
  </div>
...
function cardHTML(p){
  ...
  const rtBtn=p.lat?('<button class="btn brt" class="route-btn" data-id="'+p.id+'" onclick="event.stopPropagation();addToRoute('+p.id+')">+Route</button>'):'';
  ...
  return '<div class="card '+p.priority+(isC(p.id)?' done':'')+'" data-id="'+p.id+'" onclick="openM('+p.id+')">'
    ...
    +lastH
    +'<div class="cacts">'
      +'<button class="btn blog" class="log-call-btn" data-id="'+p.id+'" onclick="event.stopPropagation();openM('+p.id+')">Log Call</button>'
      +(last&&last.notes==='Skipped'
        ?'<button class="btn bskip" class="skip-btn" data-id="'+p.id+'" onclick="event.stopPropagation();unskip('+p.id+')" style="background:#ecfdf5;color:#059669;border-color:#6ee7b7">Unskip</button>'
        :'<button class="btn bskip" class="skip-btn" data-id="'+p.id+'" onclick="event.stopPropagation();skip('+p.id+')">Skip</button>')
      +rtBtn
    +'</div>'
  +'</div>';
}
...
function renderQueueCard(){
  ...
  if(queueIdx>=queueProspects.length){
    wrap.innerHTML=
      ...
      +'<button onclick="exitQueueMode()" style="margin-top:16px;padding:10px 20px;background:var(--navy);color:#fff;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation">Done</button>'
      +'</div>';
  ...
  let phoneHTML='';
  if(p.phone){
    const cl=p.phone.replace(/\D/g,'');
    phoneHTML='<a href="tel:'+cl+'" style="display:inline-flex;align-items:center;gap:6px;background:#059669;color:#fff;padding:10px 16px;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none;touch-action:manipulation">📞 '+p.phone+'</a>';
  } else {
    phoneHTML='<div style="font-size:11px;color:#999;padding:8px 0">No phone on file &mdash; <a href="https://www.google.com/search?q='+enc(p.name+' '+p.city+' FL phone')+'" target="_blank" style="color:var(--blu)">Google it</a></div>';
  }

  wrap.innerHTML=
    '<div style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 2px 12px rgba(0,0,0,.12)">'
    +'<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">'
      +'<div>'
        +'<div style="font-size:15px;font-weight:800;color:var(--navy);margin-bottom:3px">'+p.name+'</div>'
        +'<div style="font-size:11px;color:var(--sub)">'+p.address+', '+p.city+'</div>'
      +'</div>'
      +'<button onclick="openM('+p.id+')" style="flex-shrink:0;margin-left:8px;padding:6px 10px;border:1px solid var(--brd);border-radius:8px;background:var(--surf);color:var(--sub);font-size:10px;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation">Details \u2197</button>'
    +'</div>'
    ...
    +'<div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:10px">'
      ...
      +'<span style="font-size:9px;padding:2px 7px;border-radius:10px;background:#f8fafc;border:1px solid var(--brd);color:var(--sub)">'+( OI[lc?.outcome]||'Not contacted')+'</span>'
    +'</div>'
    +(lc?.notes?'<div style="font-size:10px;color:var(--sub);margin-bottom:8px;padding:6px 8px;background:#f5f8fa;border-radius:6px;line-height:1.5">'+lc.notes+'</div>':'')
    +'<textarea id="q-notes" placeholder="Quick note (optional)..." rows="2" style="width:100%;padding:8px;border:1px solid var(--brd);border-radius:6px;font-size:12px;font-family:inherit;resize:none;outline:none;color:#1a1a2e;background:#fff"></textarea>'
    +'</div>';
}
