...
function planMyDay(){
  ...
  if(zip.length!==5||!ZIPS[zip]){toast('Enter a valid 5-digit start ZIP first');return;}

  // Use anchor business as start point if set, otherwise use ZIP
  let slat,slon;
  ...
  let candidates=P.filter(p=>p.lat&&p.lon&&!isC(p.id));
  if(county)candidates=candidates.filter(p=>p.county===county);
  if(pri)candidates=candidates.filter(p=>p.priority===pri);
  candidates=candidates.map(p=>({...p,_d:hav(slat,slon,p.lat,p.lon)})).filter(p=>p._d<=rad);
  ...
  while(remaining.length&&usedMin<budgetMin){
    ...
    remaining.forEach((p,i)=>{
      ...
      const val=(PW[p.priority]||0)+p.score-(dMin*2);
  ...
  toast('Built '+route.length+' stops  ~'+h+'h '+m+'m');
  const hint=document.getElementById('rhint');
  if(hint)hint.textContent=route.length+' stops planned  -  est. '+h+'h '+m+'m of '+hours+'h budget. Tap stops in list to add/remove.';
}
...
function clearAnchor(){
  ...
  if(rzip)rzip.placeholder='e.g. 33701';
...
function initSettings(){
  ...
  const rzip=document.getElementById('rzip');
  if(rzip&&s.home_zip&&!rzip.value)rzip.value=s.home_zip;
}
